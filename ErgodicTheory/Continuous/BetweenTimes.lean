...
* `ErgodicTheory.ae_tendsto_flowError_zero`: the one-step fluctuation control evaluated along the
  integer orbit, `n⁻¹ · (g (φ (n:ℝ) x) + g' (φ (n:ℝ) x))`, tends to `0` almost everywhere.
* `ErgodicTheory.tendsto_log_norm_atTop_of_discrete`: if the integer-time average

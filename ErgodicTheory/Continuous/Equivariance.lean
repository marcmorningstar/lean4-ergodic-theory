...
`ErgodicTheory.IsOseledetsFiltration.ae_mem_iff_limsup_le`: a vector lies in the level `V i` iff it
...
* `ErgodicTheory.norm_clm_pos`: for invertible `M` and nonzero `v`, `0 < ‖toEuclideanCLM M v‖`.
* `ErgodicTheory.ae_tendsto_logNorm_fixedTime_zero`: almost everywhere, the fixed-time log-norms
  `n⁻¹ · log ‖A t₀ (φ (n:ℝ) x)‖` and `n⁻¹ · log ‖(A t₀ (φ (n:ℝ) x))⁻¹‖` tend to `0`.
* `ErgodicTheory.glim_shift`: almost everywhere, the discrete-time growth `limsup` is unchanged by
  applying `toEuclideanCLM (A t₀ x)` to the test vector.
* `ErgodicTheory.ae_flow_equivariant`: almost everywhere, the Oseledets filtration is equivariant

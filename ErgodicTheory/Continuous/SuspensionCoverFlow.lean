...
`ErgodicTheory.Continuous.SuspensionFlowCocycle` to the **cover** `X × ℝ` of the suspension (mapping
...
* `ErgodicTheory.coverCocycle`: `coverCocycle A T hτ hc hcpos p t` is the matrix accumulated by the
...
* `ErgodicTheory.coverCocycle_base`: on the base section (`h = 0`) the cover cocycle agrees with the
  cross-section flow cocycle, `coverCocycle (x, 0) t = flowCocycleSection t x`.
* `ErgodicTheory.coverCocycle_section_returnTime`: the **flow-cocycle multiplicativity along the
...
`ErgodicTheory.Continuous.SuspensionCocycle`). The present file lands the cover *definition*, its

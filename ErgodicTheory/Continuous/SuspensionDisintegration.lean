...
on the suspension quotient space `ErgodicTheory.SuspensionSpace` (against its invariant probability
measure `ErgodicTheory.suspensionMeasure`). It is the disintegration / fundamental-domain Fubini
correspondence flagged as the open keystone in `ErgodicTheory.Continuous.SuspensionMeasureTransfer`.
...
* `ErgodicTheory.suspensionMeasure_ae_iff`: `μ̂`-a.e.-`Q` ↔ box-a.e.-`Q ∘ π`, for measurable `Q`.
* `ErgodicTheory.ae_suspensionMeasure_of_ae_restrict`: box-a.e.-`Q ∘ π` ⇒ `μ̂`-a.e.-`Q`.
* `ErgodicTheory.ae_restrict_suspensionDomain_of_ae_base`: base-`μ`-a.e.-`P` ⇒ box-a.e.-`P ∘ fst`.
* `ErgodicTheory.ae_suspensionMeasure_of_ae_base`: base-`μ`-a.e.-`(Q ∘ π factored through fst)` ⇒
  `μ̂`-a.e.-`Q`.
* `ErgodicTheory.ae_suspensionMeasure_section_exponent_set`: the composition applied to the
...
`SuspensionSpace`. As documented in `ErgodicTheory.Continuous.SuspensionMeasureTransfer`, the matrix

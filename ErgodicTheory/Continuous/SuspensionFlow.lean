...
* `ErgodicTheory.suspensionTranslate`: the `ℝ`-translation `S t (x, s) = (x, s + t)` on `X × ℝ`.
* `ErgodicTheory.suspensionFlowMap`: the descended time-`t` map `ζ_t` on the suspension space.

## Main results

* `ErgodicTheory.measurePreserving_translate`: `S t` preserves `μ × volume` (fibrewise translation
  invariance of Lebesgue measure).
* `ErgodicTheory.suspensionAct_translate`: the commutation
  `suspensionAct n (S t p) = S t (suspensionAct n p)` of the action with the translation — the
  well-definedness core.
* `ErgodicTheory.suspensionFlowMap_mk`: the descent identity `ζ_t [p] = [S t p]`.
* `ErgodicTheory.suspensionFlowMap_zero`: `ζ_0 = id`.
* `ErgodicTheory.suspensionFlowMap_add`: `ζ_(s+t) = ζ_s ∘ ζ_t`.
* `ErgodicTheory.measurable_suspensionFlowMap`: each `ζ_t` is measurable.

...
The construction sits on top of `ErgodicTheory.Continuous.SuspensionFlowCocycle`
(`flowCocycleSection`, `flowCocycleSection_returnTime`) and the base cocycle identity of
`ErgodicTheory.Cocycle.Basic` (`cocycle_add`, `cocycle_one`).

## Main results

* `ErgodicTheory.flowCocycleSection_returnTime_add`: sampling the section flow cocycle at the
...
* `ErgodicTheory.flowCocycleSection_returnTime_succ`: the one-return step — sampling at the

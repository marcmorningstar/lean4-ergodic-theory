...
`ErgodicTheory.ae_suspensionMeasure_hasFlowExponent`
(`ErgodicTheory.Continuous.SuspensionSpaceExponentValue`) and ties the resulting `μ̂`-a.e. flow
exponent to the genuine measure-preserving flow `ErgodicTheory.suspensionFlow`
(`ErgodicTheory.Continuous.SuspensionFlowMP`).
...
`ErgodicTheory.Continuous.SuspensionQuotientImage` discharges exactly that input unconditionally
...
* `ErgodicTheory.ae_suspensionMeasure_hasFlowExponent_flowOrbit`: the **flow-tied** corollary. For
...
(`ErgodicTheory.Continuous.SuspensionQuotientImage`): the lifted exponent set on the quotient is the

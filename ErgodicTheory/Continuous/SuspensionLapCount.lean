...
`ErgodicTheory.Continuous.SuspensionCocycle`:
...
* `ErgodicTheory.returnTime_strictMono`: `n ↦ returnTime n x` is `StrictMono` (positive roof).
* `ErgodicTheory.returnTime_tendsto_atTop`: the return times diverge to `+∞`.
* `ErgodicTheory.lapCount_returnTime_le`: `returnTime (lapCount t x) x ≤ t` for `0 ≤ t`.
* `ErgodicTheory.lapCount_lt_returnTime_succ`: `t < returnTime (lapCount t x + 1) x`.

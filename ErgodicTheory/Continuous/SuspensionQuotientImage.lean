...
* `ErgodicTheory.suspensionActEquiv`: the suspension action `suspensionAct T hτ n` packaged as a
  `MeasurableEquiv` of `X × ℝ`, with inverse `suspensionAct T hτ (-n)`.
* `ErgodicTheory.measurableSet_suspensionAct_image`: `S` measurable ⇒ `suspensionAct T hτ n '' S`
  measurable.
* `ErgodicTheory.preimage_image_suspensionMk`: the orbit-saturation identity
  `suspensionMk ⁻¹' (suspensionMk '' S) = ⋃ n : ℤ, suspensionAct T hτ n '' S`.

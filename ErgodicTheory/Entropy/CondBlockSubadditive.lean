...
`ErgodicTheory.Entropy.CondKSEntropy`:
...
* `ErgodicTheory.Entropy.condEntropy_ksJoin_append_le`: block-subadditivity of the conditional
  iterated-join entropy for an arbitrary sub-σ-algebra `ℱ`.
* `ErgodicTheory.Entropy.condEntropy_ksJoin_le_nsmul_log_card`: the uniform linear bound

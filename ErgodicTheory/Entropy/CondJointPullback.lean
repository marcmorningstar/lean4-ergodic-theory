...
* `ErgodicTheory.Entropy.condEntropy_comap_preimage`: the single-map joint pull-back
  `H(S⁻¹P | comap S 𝒜) = H(P | 𝒜)`.
* `ErgodicTheory.Entropy.condEntropy_comap_pullback`: the iterated form for `S = T^[n]`.

...
* `ErgodicTheory.Entropy.condKsEntropySeq_subadditive`: subadditivity of the conditional sequence.
* `ErgodicTheory.Entropy.condKsSubadditive`: the sequence is a `Subadditive` sequence.
* `ErgodicTheory.Entropy.tendsto_condKsEntropySeq`: convergence to the Fekete limit.
* `ErgodicTheory.Entropy.condKsEntropyPartition_le_condEntropy`: `h(α, T | 𝒜) ≤ H(α | 𝒜)`.
* `ErgodicTheory.Entropy.condKsEntropyPartition_bot`: conditioning on `⊥` recovers `h(α, T)`.

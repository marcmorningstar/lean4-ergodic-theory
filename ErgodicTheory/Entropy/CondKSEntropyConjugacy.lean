...
matched by pull-back. It is the conditional companion of `ErgodicTheory.Entropy.FactorEntropy`
(`factor_relative_eq`) and `ErgodicTheory.Entropy.KSEntropyConjugacy`
...
(`ErgodicTheory.Entropy.CondJointPullback`); a `comap e 𝒜β`-set is *literally* `e ⁻¹' A'` for an
...
(`ErgodicTheory.Entropy.CondExpEquivariant`), generalised from a self-map to a measure-preserving
...
(`ErgodicTheory.Entropy.CondJointPullback`). -/

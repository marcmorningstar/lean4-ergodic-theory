...
h(α, T | 𝒜)` from `ErgodicTheory.Entropy.CondKSEntropy`, this file defines the **relative
...
partitions `α`. This is the conditional mirror of `ErgodicTheory.Entropy.KSEntropySystem`.
...
* `ErgodicTheory.Entropy.condKsEntropy`: the relative Kolmogorov–Sinai entropy `h(T | 𝒜)` of a

...
`ErgodicTheory.Entropy.Partition` to the **conditional** setting. Given a probability space `(α, μ)`
...
* `ErgodicTheory.Entropy.condEntropy`: the conditional Shannon entropy
...
* `ErgodicTheory.Entropy.condEntropy_bot`: conditioning on the trivial σ-algebra `⊥` recovers the
  ordinary entropy.
* `ErgodicTheory.Entropy.condEntropy_le`: conditioning does not increase entropy.

...
`ErgodicTheory.Entropy.CondPartition` (which defines `condEntropy μ 𝒜 s`, the `μ`-average of the
...
* `ErgodicTheory.Entropy.condEntropy_join_le`: subadditivity of conditional entropy under joins.

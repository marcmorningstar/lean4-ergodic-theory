...
(`ErgodicTheory.Entropy.IsFactorMap`), every finite measurable partition `R` of the *target* `β`
...
`h(π⁻¹R, T) = h(R, S)`  (`ErgodicTheory.Entropy.factor_relative_eq`),
...
* `ErgodicTheory.Entropy.factor_relative_eq`: `ksEntropyPartition hT (R.pulledBack hπ) =

...
`π ∘ T = S ∘ π`. This file packages that data as `ErgodicTheory.Entropy.IsFactorMap` and proves the
...
* `ErgodicTheory.Entropy.IsFactorMap.invariant_comap`: the pulled-back σ-algebra `comap π 𝓑` is
  `T`-invariant, `comap T (comap π 𝓑) ≤ comap π 𝓑` — the hypothesis the entropy machinery needs.
* `ErgodicTheory.Entropy.IsFactorMap.measurable_comap_le`: `comap π 𝓑` is a genuine sub-σ-algebra of

...
* `ErgodicTheory.Entropy.condEntropy_reindex`: invariance of `condEntropy` under reindexing the cell
  family by an equivalence of finite index types.
* `ErgodicTheory.Entropy.entropy_joinCells_comm`: symmetry `H(α ∨ β) = H(β ∨ α)` of the static join
  entropy.
* `ErgodicTheory.Entropy.condEntropy_ksJoin_le_sum`: finite conditional subadditivity of the
  iterated-join entropy, `H(A_n | 𝒞) ≤ ∑ₖ<ₙ H(T⁻ᵏα | 𝒞)`, for an arbitrary fixed sub-σ-algebra `𝒞`.
* `ErgodicTheory.Entropy.ksEntropySeq_le_add_condEntropy`: the per-`n` bound
  `H(A_n) ≤ H(B_n) + n · H(α | σ(β))`.
* `ErgodicTheory.Entropy.ksEntropyPartition_le_add_condEntropy`: Le Maître's inequality (7),
  `h(α, T) ≤ h(β, T) + H(α | σ(β))`.

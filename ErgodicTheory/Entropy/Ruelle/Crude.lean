...
It validates the whole covering pipeline (`ErgodicTheory.MeasureTheory.CoveringFromVolume` +
`ErgodicTheory.Entropy.Ruelle.AtomCount`) by assembling the *scalar arithmetic backbone* of the
...
* `ErgodicTheory.crudeRuelle_le_log_deriv_rate` — the crude Ruelle bound `h(P, T) ≤ d · B` from the

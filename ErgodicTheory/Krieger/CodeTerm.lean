...
(the convention of `ErgodicTheory.Krieger.sentinelEncode`, which appends `[s]`). For a point `x` at
...
The forward/backward searches `fwdSentinel`/`bwdSentinel` of `ErgodicTheory.Krieger.TowerCode` are
...
no closer coordinate `-1 - n`, `n < d`. (Anchor `-1`, as in `ErgodicTheory.Krieger.blockStart`/
`blockOffset`.) -/
...
closer coordinate `n`, `n < f`. (Anchor `0`, as in `ErgodicTheory.Krieger.blockEnd`.) -/
...
(`ErgodicTheory.Krieger.parse_event_cannot_separate`) was missing. -/
...
a.e. correctness is carried, exactly as `ErgodicTheory.Krieger.ColumnCodeData` carries theirs. -/
...
This is the column-tiling analogue of `ErgodicTheory.Krieger.ColumnCodeData`: it is exactly the

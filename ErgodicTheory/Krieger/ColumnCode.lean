...
`ErgodicTheory.Krieger.Recovery`: the **measurable bi-infinite sentinel parser**
...
position-aware `SentinelColumnCodeAt` in `ErgodicTheory.Krieger.TowerCode`).
...
obstruction is formalized in `ErgodicTheory.Krieger.parse_event_cannot_separate` (`TowerCode.lean`).

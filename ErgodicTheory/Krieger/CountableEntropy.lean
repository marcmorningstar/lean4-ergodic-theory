...
repository's existing entropy stack (`ErgodicTheory.Entropy.*`) is indexed by a `Fintype`; this file
...
Following `ErgodicTheory.Entropy.entropy`, the entropy is defined on *loose data* — a
...
`ErgodicTheory.Entropy.MeasurePartition` (finite) for the agreement lemma.
...
* `ErgodicTheory.Krieger.cHμ`: the countable Shannon entropy `∑' i, negMulLog (μ (s i)).toReal` of a
...
`cHμ` agrees with the repository's finite `ErgodicTheory.Entropy.entropy`: the unconditional `tsum`
...
lemma cHμ_eq_entropy [Fintype ι] (μ : Measure α) (s : ι → Set α) :
    cHμ μ s = ErgodicTheory.Entropy.entropy μ s := by
  rw [cHμ_def, ErgodicTheory.Entropy.entropy_def, tsum_fintype]

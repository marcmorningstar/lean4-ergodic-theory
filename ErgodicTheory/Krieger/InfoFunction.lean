...
* `ErgodicTheory.Krieger.itinerary`: the `n`-step least-index code `f : Fin n → ι` of a point.
* `ErgodicTheory.Krieger.atomOf`: the atom `⋂ₖ T⁻ᵏ (P_{f k})` of the iterated join containing `x`.
* `ErgodicTheory.Krieger.infoFun`: the information function `iₙ(x) = -log μ(atom of x)`.

## Main results

* `ErgodicTheory.Krieger.itinerary_spec`: `Tᵏ x ∈ P_{itinerary x k}` for all `k`.
* `ErgodicTheory.Krieger.mem_atomOf`: `x ∈ atomOf x`.
* `ErgodicTheory.Krieger.measurableSet_atomOf`: the atom is measurable.
* `ErgodicTheory.Krieger.measurable_infoFun`: the information function is measurable.
* `ErgodicTheory.Krieger.infoFun_nonneg`: the information function is nonnegative.
* `ErgodicTheory.Krieger.integral_infoFun_eq`: `∫ iₙ dμ = H(⋁ₖ₌₀ⁿ⁻¹ T⁻ᵏ P) = ksEntropySeq …`.

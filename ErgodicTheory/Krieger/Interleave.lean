...
`ErgodicTheory.Krieger.sentinelParseAt_itin_eq` (the dynamical headline of
`ErgodicTheory.Krieger.CodeTerm`) into the `recovers_tiled` field of
`ErgodicTheory.Krieger.ColumnLayoutData`, hence sub-problem B's `CodesTwoSidedMod0c`.
...
(`ErgodicTheory.Krieger.rokhlin_tower`), so a single tower gives recovery only mod-`ε`. The mod-0
...
(`ErgodicTheory.Krieger.eventually_mem_of_summable_compl`) a.e. `x` is eventually inside the
...
read via `ErgodicTheory.Krieger.floorAddr` + `ErgodicTheory.Krieger.exists_sentinelEncoding`) whose
...
`RefiningTowerCode.toColumnLayoutData` with `ErgodicTheory.Krieger.ColumnLayoutData.codes` gives
`CodesTwoSidedMod0c` for the code partition — the deliverable that slots into
`ErgodicTheory.Krieger.KriegerCodingData.code_codes`. The entire symbolic side of sub-problem B thus

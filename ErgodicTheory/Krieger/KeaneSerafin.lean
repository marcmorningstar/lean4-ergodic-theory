...
assembles into the `KeaneSerafinData` of `ErgodicTheory.Krieger.Generator` — the dynamical input to
the finite-entropy countable two-sided generator. **Unconditional and sorry-free.** -/
...
`ksEntropyPartition_eq_condEntropy_iSup` (`ErgodicTheory.Krieger.SMBSharp`), the SMB development
gives the full pointwise a.e. SMB `ae_tendsto_div_infoFun_self` and the in-measure equipartition

...
the information functions `iₙ(x) = -log μ(atomₙ(x))` (built in `ErgodicTheory.Krieger.InfoFunction`)
satisfy
`(1/n)·iₙ(x) → h(P,T) = ksEntropyPartition`  for `μ`-a.e. `x`.

## What is proved here (sorry-free)

* `ErgodicTheory.Krieger.ae_limsup_div_infoFun_le_log_card` — the **crude name-count upper bound**:
...
expectation: `ErgodicTheory.Entropy.condCandidate B tⱼ` (`CondGivenPartitionBridge.lean`) is the

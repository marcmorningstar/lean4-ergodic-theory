...
`ErgodicTheory.Krieger.SMBPointwise`, making the headline a.e. convergence
...
* **Leaf 2 (Maker/Breiman dominated-Cesàro).** `makerTail`: from `g* ∈ L¹` and the a.e. Lévy limit
  ...
  `m⁻¹·h(T^[m]x) → 0` (`ErgodicTheory.ae_tendsto_orbit_div_atTop_zero`) for the small-lag part.

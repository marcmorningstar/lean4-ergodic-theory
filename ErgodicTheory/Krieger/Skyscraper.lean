...
* `ErgodicTheory.Krieger.skyscraper e A` — the skyscraper over `A`.
* `ErgodicTheory.Krieger.sweepOut e A` — the forward sweep-out `⋃ j, eʲ '' A`.
* `ErgodicTheory.Krieger.preimageSweep e A` — the orbit-hits-`A` set `⋃ n, eⁿ ⁻¹' A`.

...
* `ErgodicTheory.Krieger.towerBase e A N` — the union of floors `e^[m·N] '' (returnLevel e A (k+1))`
  that begin a complete height-`N` block (guard `(m+1)·N ≤ k+1`).
* `ErgodicTheory.Krieger.coveredSet e A N` — the union `⋃ i : Fin N, eⁱ '' (towerBase e A N)`.
* `ErgodicTheory.Krieger.topFloors e A N` — the top incomplete floors `⌊(k+1)/N⌋·N ≤ j ≤ k` of each
  column.

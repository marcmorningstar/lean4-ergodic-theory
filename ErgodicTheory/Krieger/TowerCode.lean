...
`ErgodicTheory.Krieger.ColumnCodeData` exactly as before, but now with a recovery field that is
...
partition — the deliverable that slots into `ErgodicTheory.Krieger.KriegerCodingData.code_codes`. -/

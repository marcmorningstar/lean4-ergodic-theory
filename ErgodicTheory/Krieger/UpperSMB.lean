...
theorem (`ErgodicTheory.Krieger.SMBLeaves.ae_tendsto_div_infoFun`) at the level of the *concrete*
...
* `ErgodicTheory.Krieger.infoFun_succ_ae` — the one-step front-peel of the Breiman telescoping:
  `infoFun_{n+1}(x) = g_n(x) + infoFun_n(Tx)` a.e.
* `ErgodicTheory.Krieger.ae_tendsto_div_infoFun_self` — the pointwise SMB for `infoFun`:
  `(1/n)·infoFunₙ(x) → h(P,T)` a.e. for ergodic `T`.
* `ErgodicTheory.Krieger.upperSMBInMeasure_of_ergodic` — `UpperSMBInMeasure hT P` for ergodic `T`:
...
theorem ae_tendsto_div_infoFun_self (herg : Ergodic T μ) (P : MeasurePartition μ ι) :
  ...
  have hdecay : ∀ᵐ x ∂μ, Tendsto
      ...
      atTop (𝓝 0) :=
    ErgodicTheory.ae_tendsto_orbit_div_atTop_zero hT hg0int

...
`ErgodicTheory.Krieger.StageBuild` reduced sub-problem B of the unconditional Krieger theorem to
...
The genuine crux (cf. `ErgodicTheory.Krieger.RefTower`): summable misses `∑ₘ μ (T m)ᶜ < ∞`
...
the deliverable that slots into `ErgodicTheory.Krieger.KriegerCodingData.code_codes`. The
entire residual

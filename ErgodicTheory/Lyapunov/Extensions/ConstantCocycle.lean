...
`ErgodicTheory.exp_exponents_eq_eigenvalues₀_oseledetsLimit` gives `exp (exponents … i) = |M|`'s
`i`-th sorted eigenvalue, hence `exponents … i = log` of the `i`-th largest modulus.

## Main definitions

* `ErgodicTheory.absMatrix` — the absolute value `|M| = cfc |·| M` of a (real) matrix, the Oseledets
...
* `ErgodicTheory.exponents_const` — for symmetric invertible `M`,

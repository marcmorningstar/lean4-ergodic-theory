...
theory (`ErgodicTheory.tendsto_gammaK`, `ErgodicTheory.exists_lam_tendsto_singularValue`,
`ErgodicTheory.exponents`) the partial-sum limits `Γ_k` and the per-`σ` Lyapunov exponents
...
(`ErgodicTheory.tendsto_kingman_ergodic`) for the non-ergodic one (`ErgodicTheory.tendsto_kingman`):
...
`ErgodicTheory.tendsto_gammaK_of_integrableLogNorm` and
`ErgodicTheory.exists_lam_tendsto_singularValue`.

## Main results

* `ErgodicTheory.tendsto_gammaK_nonergodic` — the partial-sum limit `Γ_k` as a `T`-invariant
  integrable function `G : X → ℝ`, with `(1/n) log sprod_k → G` almost everywhere.
* `ErgodicTheory.exists_exponents_nonergodic` — the full Lyapunov spectrum as a family of
  `T`-invariant integrable functions `lam : ℕ → X → ℝ`, each the a.e. limit of
  `(1/n) log σᵢ(A⁽ⁿ⁾)`.
* `ErgodicTheory.exists_sumPosExp_nonergodic` — the sum of the positive exponents, as a

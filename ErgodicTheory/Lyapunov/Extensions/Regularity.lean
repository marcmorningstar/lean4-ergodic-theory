...
* `ErgodicTheory.gammaKInf` — the Fekete infimum `⨅ n, (∫ log sprod_k(n+1))/(n+1)`.
* `ErgodicTheory.botExp` — the bottom exponent `λ_d = Γ_d − Γ_{d-1}`.

## Main results

* `ErgodicTheory.integral_logSprod_subadditive` — the integral sequence is subadditive.
* `ErgodicTheory.tendsto_integral_logSprod` — Fekete: the normalized integral sequence converges to
  `gammaKInf`.
* `ErgodicTheory.gammaK_eq_gammaKInf`, `ErgodicTheory.gammaK_eq_iInf` — the ergodic constant is the
  Fekete infimum.
* `ErgodicTheory.tendsto_integral_logSprod_of_dominated` — per-`n` integral continuity under a fixed
  log-integrable envelope (regime 1).
* `ErgodicTheory.gammaK_upperSemicontinuous` — USC of the top-`k` partial-sum growth rate `Γ_k`.
* `ErgodicTheory.topExponent_upperSemicontinuous` — USC of the top exponent `λ₁ = Γ_1`.
* `ErgodicTheory.botExp_eq_exponents_last`, `ErgodicTheory.botExp_lowerSemicontinuous` — the bottom
...
See also `ErgodicTheory.gammaK_one_eq_topExponent` in `ExteriorCocycle`. -/

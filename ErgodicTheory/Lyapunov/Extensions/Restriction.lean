...
* `InvariantSubbundle` — a measurable, a.e. cocycle-invariant subbundle, with the
  ...
  matching `ErgodicTheory.IsOseledetsFiltration` / `vflag_equivariant`.
...
`MeasurableSubspace.inf` (`ErgodicTheory/TwoSided/MeasurableInf.lean`).
...
`ErgodicTheory.IsOseledetsFiltration` and `vflag_equivariant`. -/

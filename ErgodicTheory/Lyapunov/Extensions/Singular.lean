...
* `ErgodicTheory.isSubadditiveCocycle_posLogNorm` — `log⁺‖A⁽ⁿ⁾‖` is a subadditive cocycle, no
  invertibility.
* `ErgodicTheory.integrable_posLogNorm_cocycle`, `ErgodicTheory.bddBelow_posLogNorm` — the Kingman
  provisos, discharged from `IntegrableLogNorm A μ` alone.
* `ErgodicTheory.tendsto_top_posLogNorm` — the a.e.-constant forward top value `λ₁⁺`.
* `ErgodicTheory.limsup_logNorm_le_top` — the upper bound
  `limsup (1/n) log‖A⁽ⁿ⁾‖ ≤ λ₁⁺`.
* `ErgodicTheory.isSubadditiveCocycle_posLogSprod`, `ErgodicTheory.integrable_posLogSprod`,
  `ErgodicTheory.tendsto_top_posLogSprod`, `ErgodicTheory.limsup_logSprod_le_top` — the analogous
...
`ErgodicTheory.furstenbergKesten_norm`). -/
...
`limsup`s agree (`Filter.limsup_congr`), and the latter equals `λ₁⁺` (`limsup_eq_liminf_posLogNorm`
...
theorem limsup_logNorm_eq_top_of_pos [IsProbabilityMeasure μ] (hT : Ergodic T μ)
    {A : X → Matrix (Fin d) (Fin d) ℝ} (hAmeas : Measurable A) (hint : IntegrableLogNorm A μ) :
    ∃ lam : ℝ,
      (∀ᵐ x ∂μ, Tendsto (fun n : ℕ => (n : ℝ)⁻¹ * Real.posLog ‖cocycle A T n x‖) atTop (𝓝 lam)) ∧
      (0 < lam → ∀ᵐ x ∂μ,
        Filter.limsup
          (fun n : ℕ => (((n : ℝ)⁻¹ * Real.log ‖cocycle A T n x‖ : ℝ) : EReal)) atTop
          = (lam : EReal)) := by

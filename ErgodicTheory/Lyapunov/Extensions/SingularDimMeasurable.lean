...
(`ErgodicTheory.cocycleRank_add_le_right`: the rank cannot increase as the cocycle
...
`ErgodicTheory.Lyapunov.Extensions.SingularRankMinor` supplied the per-step measurable
dimension function `measurable_cocycleRank`. This file **closes the eventual
dimension data**:

* `ErgodicTheory.eventualRank A T x := ⨅ n, cocycleRank A T n x` — the stabilised rank;
* `ErgodicTheory.measurable_eventualRank` — it is measurable (a countable infimum of the
  measurable `ℕ`-valued rank functions);
* `ErgodicTheory.eventualKerDim A T x := d - eventualRank A T x` and
  `ErgodicTheory.measurable_eventualKerDim` — the eventual-kernel dimension is measurable.
...
* `ErgodicTheory.eventualRank`: `⨅ n, cocycleRank A T n x`, the stabilised cocycle rank.
* `ErgodicTheory.eventualKerDim`: `d - eventualRank A T x`, the eventual-kernel dimension.

## Main results

* `ErgodicTheory.le_eventualRank_iff`: `k ≤ eventualRank A T x ↔ ∀ n, k ≤ cocycleRank A T n x`.
* `ErgodicTheory.measurableSet_le_eventualRank`: `{x | k ≤ eventualRank A T x}` is measurable.
* `ErgodicTheory.measurable_eventualRank`: `x ↦ eventualRank A T x` is measurable.
* `ErgodicTheory.measurable_eventualKerDim`: `x ↦ eventualKerDim A T x` is measurable.
-/

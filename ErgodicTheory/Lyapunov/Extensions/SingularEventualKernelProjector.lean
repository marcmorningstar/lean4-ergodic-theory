...
* `ErgodicTheory.eventualKerEuclid`: the eventual kernel transported to `EuclideanSpace ℝ (Fin d)`,
  `⨆ n, cocycleKerEuclid A T n x`.

## Main results

* `ErgodicTheory.cocycleKerEuclid_mono`: the step-kernel family `n ↦ cocycleKerEuclid A T n x` is
  monotone (transported `cocycleKer_mono`).
* `ErgodicTheory.tendsto_starProjection_cocycleKerEuclid`: for each vector `v`, the finite-step
...
* `ErgodicTheory.measurableSubspace_eventualKer`:

...
`ErgodicTheory.forwardSingularExponent`). Here `sprod_k = ErgodicTheory.sprod A T k` is the top-`k`
...
* `ErgodicTheory.measurable_forwardSingularExponentLog` — `γ_k^log` is measurable.
* `ErgodicTheory.forwardSingularExponentLog_le` — `γ_k^log(x) ≤ γ_k(x)` for **every** `x`
...
* Everything here rests only on `ErgodicTheory.sprod` and the `EReal`-`limsup`/`Real.log`
...
* The companion `log⁺` exponent (`ErgodicTheory.forwardSingularExponent`) is `μ`-a.e. a finite real
  ...
  (expanding) stratum where `γ_k > 0` (cf. `ErgodicTheory.limsup_logSprod_eq_top_of_pos`); on the
...
`log⁺` exponent `ErgodicTheory.forwardSingularExponent`. Termwise `Real.log t ≤ Real.posLog t`, so

...
* `ErgodicTheory.cocycle_succ_mulVec`: the **kernel-shift identity**
...
* `ErgodicTheory.mapsTo_cocycleKer`: the **per-step equivariance** —
  `A x` maps `cocycleKer A T (n+1) x` into `cocycleKer A T n (T x)`.
* `ErgodicTheory.eventualKer_equivariant`: the **headline equivariance of the kernel stratum**

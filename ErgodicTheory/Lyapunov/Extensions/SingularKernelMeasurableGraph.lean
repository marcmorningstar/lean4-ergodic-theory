...
* `ErgodicTheory.mem_eventualKer`: the **directed-union characterization** —
  `v ∈ eventualKer A T x ↔ ∃ n, (cocycle A T n x) *ᵥ v = 0` (via `mem_iSup_of_directed`
  on the monotone step-kernel chain and `mem_cocycleKer`).
* `ErgodicTheory.measurable_cocycleMulVec`: the map
  `(x, v) ↦ (cocycle A T n x) *ᵥ v` is measurable on `X × (Fin d → ℝ)`.
* `ErgodicTheory.measurableSet_graph_cocycleKer`: each per-step graph
  `{(x, v) | (cocycle A T n x) *ᵥ v = 0}` is measurable.
* `ErgodicTheory.measurableSet_graph_eventualKer`: the **measurable graph of the
  eventual-kernel family** `{(x, v) | v ∈ eventualKer A T x}` (a countable union of the
  per-step graphs).
* `ErgodicTheory.measurableSet_mem_eventualKer`: for a fixed `v`, the section

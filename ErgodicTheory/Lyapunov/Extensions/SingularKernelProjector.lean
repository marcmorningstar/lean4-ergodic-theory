...
`ErgodicTheory.MeasurableSubspace` notion of `ErgodicTheory.Lyapunov.MeasurableSubspace`).
...
`ErgodicTheory.measurable_spectralProjector` (no new measurable-selection theorem is needed). Set
...
* `S x` is Hermitian and positive semidefinite (`Matrix.isHermitian_conjTranspose_mul_self`,
  ...
  `ErgodicTheory.instMeasurableMul₂Matrix`).
...
* `ErgodicTheory.cocycleKerEuclid`: the `n`-step cocycle kernel transported to a subspace of
...
measurable (`ErgodicTheory.instMeasurableMul₂Matrix`). -/

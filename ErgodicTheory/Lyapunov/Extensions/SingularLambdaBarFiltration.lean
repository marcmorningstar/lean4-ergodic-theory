...
`lambdaBar A T x v = limsup_n (1/n)·log‖A⁽ⁿ⁾(x)·v‖` (`ErgodicTheory.lambdaBar`,
`ErgodicTheory/Lyapunov/GrowthFunction.lean`).
...
* `ErgodicTheory.lambdaBar_step_le`: the **floored** det-free cocycle-step inequality
...
* `ErgodicTheory.lambdaBar_add_le_max_zero`: the floored non-Archimedean inequality.
* `ErgodicTheory.isUltrametricGrowth_max_lambdaBar`: `v ↦ max (lambdaBar A T x v) 0` is an
  `IsUltrametricGrowth` function.
* `ErgodicTheory.lambdaBarSublevel`: the sublevel **submodule** at threshold `c`.

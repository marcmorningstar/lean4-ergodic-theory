...
records that rank as the function `ErgodicTheory.cocycleRank`, together with the basic
...
* `ErgodicTheory.cocycleRank`: the rank `(cocycle A T n x).rank` of the `n`-step cocycle —
  the number of non-collapsed directions after `n` steps.

## Main results

* `ErgodicTheory.cocycleRank_le`: `cocycleRank A T n x ≤ d` (rank is bounded by the
  ambient dimension).
* `ErgodicTheory.cocycleRank_zero`: `cocycleRank A T 0 x = d` (the zero-step cocycle is the
  identity, of full rank).
* `ErgodicTheory.cocycleRank_add_le_left` / `..._right` / `..._min`: the rank-drop

...
a *fixed* slow space, e.g. `ErgodicTheory.limsup_le_of_mem_vslow`); it does **not** lower-bound the
...
* `ErgodicTheory.vSlowSingularStep` — the `n`-step slow approximant (range of `1 − bandProjector`).

...
(`ErgodicTheory.singularDirExponent`) is **NOT** antitone — the positive-part clamp `log⁺` resets the
...
* `ErgodicTheory.singularSpectralValue_antitone` — **the headline**: `λ_k^gen` is antitone in `k` for
  **every** `x` (deterministic), since the singular values are antitone and `ENNReal.log` is
  monotone.
* `ErgodicTheory.singularSpectralValue_le_genLog_sub` / `ErgodicTheory.singularSpectralValue_succ_telescope`
  — the telescoping tie to the cumulative genuine-`log` exponent
  `ErgodicTheory.forwardSingularExponentLog`: where the top-`k` volume is positive, the per-direction
  exponent is the cumulative increment `γ_{k+1}^log − γ_k^log`.
...
measurable `EReal`-difference: `sprod` is measurable (`ErgodicTheory.measurable_sprod`, `[NeZero d]`),
...
term is `≤ (1/n) log⁺‖A⁽ⁿ⁾‖` (`ErgodicTheory.singularSpectralValue_term_le_posLogNorm`), and the latter
converges `μ`-a.e. to the finite forward top value `λ₁⁺` (`ErgodicTheory.tendsto_top_posLogNorm`). So
...
`ErgodicTheory.cocycleSublevelEuclid` a **squared-singular-value** threshold `t` strictly between the
...
`j`-th gap). Built by choosing each cut independently with `ErgodicTheory.exists_cutThreshold` on the

...
* `ErgodicTheory.inner_slow_band_le`: the slow-band quadratic form is `≤ c^{2n}‖w‖²`.
* `ErgodicTheory.rpow_mem_spectrum_qpow_le`: every `qpow` spectral value `t` has
  `t^{2n} ≤ ‖A⁽ⁿ⁾‖²`.
* `ErgodicTheory.inner_fast_band_le`: the fast-band quadratic form is
  `≤ ‖A⁽ⁿ⁾‖²·‖Pₙ w‖²`.
* `ErgodicTheory.norm_sq_cocycle_apply_le_split`: the master inequality
  `‖A⁽ⁿ⁾ v‖² ≤ ‖A⁽ⁿ⁾‖²·‖Pₙ v‖² + c^{2n}·‖v‖²`.
* `ErgodicTheory.limsup_log_cocycle_apply_le_of_angle`: the spectral upper bound
...
estimate `summable_norm_bandProjector_succ_sub` (in `ErgodicTheory.Lyapunov.OseledetsLimit`)

...
The headline `dimH_eq_ksEntropy_div_log_two` (`ErgodicTheory/Multifractal/SymbolicDimension.lean`,
...
* `ErgodicTheory.Multifractal.dimH_bern_eq_Hnu_div_log_two`: for a biased Bernoulli measure `bern ν`
...
theorem dimH_bern_eq_Hnu_div_log_two (ν : Measure α₀) [IsProbabilityMeasure ν] {i j : α₀}
  ...
  have hkse : (ErgodicTheory.Entropy.ksEntropy hσ.toMeasurePreserving).toReal = Hnu ν :=
    ksEntropy_bern_eq ν
  rw [hdim, hkse]

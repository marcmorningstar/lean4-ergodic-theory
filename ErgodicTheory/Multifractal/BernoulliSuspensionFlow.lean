...
* `ErgodicTheory.MeasurePreservingFlow.ksEntropy`: the metric entropy of a measure-preserving flow,
  defined as the Kolmogorov–Sinai entropy of its time-`1` map.
* `ErgodicTheory.Multifractal.suspensionBaseProj`: the factor map `π` to the base shift.
* `ErgodicTheory.Multifractal.coordPartitionZFin`: the time-`0` coordinate partition reindexed to a
  `Fin (Fintype.card α₀)`-indexed partition (so it enters the `Fin`-indexed system entropy `iSup`).

## Main results

* `ErgodicTheory.Multifractal.measurePreserving_suspensionBaseProj`: `π` preserves the suspension
  probability measure, pushing it to `bernZ ν`.
* `ErgodicTheory.Multifractal.suspensionBaseProj_comp_flow`: the intertwining `π ∘ ζ_1 = T ∘ π`.
* `ErgodicTheory.Multifractal.suspensionFlow_bernZ_ksEntropy_pos`: the constant-roof Bernoulli
...
noncomputable def MeasurePreservingFlow.ksEntropy {μ : Measure X} [IsProbabilityMeasure μ]
  ...
  ErgodicTheory.Entropy.ksEntropy (φ.measurePreserving 1)
...
-- companion module `ErgodicTheory.Multifractal.BernoulliTwoSidedEntropy` (imported above), so the

...
(`ErgodicTheory/Multifractal/BernoulliEntropy.lean`) to the **two-sided (invertible) full shift**
`BiShift α₀ := ℤ → α₀` equipped with the two-sided i.i.d. measure `bernZ ν`
(`ErgodicTheory/Multifractal/BernoulliTwoSided.lean`). The result is the base entropy datum that the

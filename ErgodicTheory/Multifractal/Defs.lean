...
* `ErgodicTheory.Multifractal.partitionFunction`: the generalized partition function
...
* `ErgodicTheory.Multifractal.massExponent`: the mass exponent `τ(q) = log Z_q / log ε`.
* `ErgodicTheory.Multifractal.renyiDim`: the Rényi / generalized dimension `D_q = τ(q) / (q - 1)`,
...
* `ErgodicTheory.Multifractal.singularitySpectrum`: the singularity spectrum

...
* `ErgodicTheory.Multifractal.renyiDim_uniform_tendsto_dim`: the refining-limit corollary, packaging
  the constant value as an actual `Tendsto … (𝓝[Set.Ioo 0 1] 0) (𝓝 d)`.
* `ErgodicTheory.Multifractal.renyiDimMeasure_uniform_eq_dim`: the measure-level mirror.

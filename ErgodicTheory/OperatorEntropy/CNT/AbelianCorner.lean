...
def projMeasurePartition (μ : Fin d → ℝ≥0∞) (c : Fin d → Fin k) :
    ErgodicTheory.Entropy.MeasurePartition (probMeasure μ) (Fin k) where
  cells i := c ⁻¹' {i}
...
theorem joinInd_eq_indicator (μ : Fin d → ℝ≥0∞) (σ : Equiv.Perm (Fin d)) (c : Fin d → Fin k)
    ...
    joinInd σ c nn f j
      = (ErgodicTheory.Entropy.ksJoinCells (projMeasurePartition μ c).cells (⇑σ) nn f).indicator
          (fun _ => (1 : ℂ)) j := by
  have hmem : ∀ l : Fin nn,
    ...
    simp only [projMeasurePartition_cells, Set.mem_preimage, Set.mem_singleton_iff,
      Equiv.Perm.coe_pow]
  rw [joinInd]
  by_cases hj : j ∈ ErgodicTheory.Entropy.ksJoinCells (projMeasurePartition μ c).cells (⇑σ) nn f
  · rw [Set.indicator_of_mem hj]
    have hj' : ∀ l : Fin nn, j ∈ (⇑σ)^[(l : ℕ)] ⁻¹' (projMeasurePartition μ c).cells (f l) := by
      rw [ErgodicTheory.Entropy.ksJoinCells_apply, Set.mem_iInter] at hj; exact hj
...
theorem vonNeumannEntropy_corrMatrix_eq_ksEntropySeq
  ...
  set mreal : (Fin nn → Fin k) → ℝ := fun f =>
    ((probMeasure μ) (ErgodicTheory.Entropy.ksJoinCells
  ...
  have hcorr : (corrMatrix (adPerm σ) (densityOfPMF μ hμ) (projPartition c) nn).val
    ...
    · subst hgf
      rw [if_pos rfl]
      have hmf0 : mreal g = ((probMeasure μ) (ErgodicTheory.Entropy.ksJoinCells
          (projMeasurePartition μ c).cells (⇑σ) nn g)).toReal := by rw [hmreal]
      have hfin2 : ∀ i ∈ (Finset.univ : Finset (Fin d)),
          μ i * (ErgodicTheory.Entropy.ksJoinCells (projMeasurePartition μ c).cells (⇑σ) nn
      ...
      by_cases hi : i ∈ ErgodicTheory.Entropy.ksJoinCells (projMeasurePartition μ c).cells (⇑σ) nn g
  ...
  rw [ErgodicTheory.Entropy.ksEntropySeq, ErgodicTheory.Entropy.ksJoin_cells,
    ErgodicTheory.Entropy.entropy_def]
...
theorem cntEntropyPartition_eq_ksEntropyPartition
    ...
    cntEntropyPartition (adPerm σ) (densityOfPMF μ hμ) (projPartition c)
      = ErgodicTheory.Entropy.ksEntropyPartition (measurePreserving_perm μ σ hinv)
          (projMeasurePartition μ c) := by
  haveI := probMeasure_isProb μ hμ
  rw [cntEntropyPartition_eq]
  unfold ErgodicTheory.Entropy.ksEntropyPartition
  rw [Subadditive.lim]
  congr 1
...
theorem cntDynamicalEntropyAbelian_eq_ksEntropy
  ...
  · unfold cntDynamicalEntropyAbelian
    ...
    exact ErgodicTheory.Entropy.le_ksEntropy (measurePreserving_perm μ σ hinv)
      (projMeasurePartition μ c)
  · unfold ErgodicTheory.Entropy.ksEntropy
    ...
    have hseqfun : ErgodicTheory.Entropy.ksEntropySeq (measurePreserving_perm μ σ hinv)
        (projMeasurePartition μ c)
        = ErgodicTheory.Entropy.ksEntropySeq (measurePreserving_perm μ σ hinv) P := by
      funext m
      unfold ErgodicTheory.Entropy.ksEntropySeq
      rw [ErgodicTheory.Entropy.ksJoin_cells, ErgodicTheory.Entropy.ksJoin_cells, hcellfun]
    have hkspart : ErgodicTheory.Entropy.ksEntropyPartition (measurePreserving_perm μ σ hinv)
        (projMeasurePartition μ c)
        = ErgodicTheory.Entropy.ksEntropyPartition (measurePreserving_perm μ σ hinv) P := by
      unfold ErgodicTheory.Entropy.ksEntropyPartition
...
example : 0 < vonNeumannEntropy (corrMatrix (adPerm (1 : Equiv.Perm (Fin 2)))
  ...
  rw [vonNeumannEntropy_corrMatrix_eq_ksEntropySeq (fun _ : Fin 2 => (1 / 2 : ℝ≥0∞))
      half_sum_eq_one 1 (fun _ => rfl) id 1, ErgodicTheory.Entropy.ksEntropySeq,
    ErgodicTheory.Entropy.ksJoin_cells, ErgodicTheory.Entropy.entropy_def]
  apply Finset.sum_pos
  · intro f _
    have hcell : ErgodicTheory.Entropy.ksJoinCells
        (projMeasurePartition (fun _ : Fin 2 => (1 / 2 : ℝ≥0∞)) id).cells
        (⇑(1 : Equiv.Perm (Fin 2))) 1 f = {f 0} := by
      ext x
      simp only [ErgodicTheory.Entropy.ksJoinCells_apply, Set.mem_iInter, Fin.forall_fin_one,

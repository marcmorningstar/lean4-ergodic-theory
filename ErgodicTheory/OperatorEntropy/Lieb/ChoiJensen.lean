...
Effros dilation inequality `ErgodicTheory.OperatorEntropy.Lieb.hpj_affine`.  Given Kraus operators
...
* `ErgodicTheory.OperatorEntropy.Lieb.cfc_blockDiagonal`: `cfc f` distributes over a block diagonal.
* `ErgodicTheory.OperatorEntropy.Lieb.krausCol_conj_blockDiagonal`: the Kraus-column Gram identity.
* `ErgodicTheory.OperatorEntropy.Lieb.choi_neg_log_loewner`: Choi's inequality for `-log`.
-/
...
lemma cfc_blockDiagonal {ι q : Type*} [Fintype ι] [DecidableEq ι] [Fintype q] [DecidableEq q]
  ...
  rw [hYeq, ErgodicTheory.OperatorEntropy.cfc_conj _ _ hBU1 hBU2 hΛsa f, hcfcΛ, hRHS]

...
* `ErgodicTheory.OperatorEntropy.Lieb.relEntropyMat`: the trace-form relative entropy.
* `ErgodicTheory.OperatorEntropy.Lieb.relEntropyMat_jointly_convex`: Lieb's joint convexity.
* `ErgodicTheory.OperatorEntropy.Lieb.relEntropyMat_eq_relEntropy`: the bridge to the spectral
...
lemma cfc_log_kron {A : Matrix nA nA ℂ} {B : Matrix nB nB ℂ} (hA : A.PosDef) (hB : B.PosDef) :
  ...
  have hWY : (UA ⊗ₖ UB) * ((1 : Matrix nA nA ℂ) ⊗ₖ diagonal (fun j => (Real.log (eB j) : ℂ)))
    ...
    rw [hWstar, ← Matrix.mul_kronecker_mul, ← Matrix.mul_kronecker_mul, Matrix.mul_one,
      hUAs', ← hLB]
  rw [hAB, ErgodicTheory.OperatorEntropy.cfc_conj (UA ⊗ₖ UB)
      (diagonal (fun i => (eA i : ℂ)) ⊗ₖ diagonal (fun j => (eB j : ℂ))) hW1 hW2 hMdiag Real.log,
    hDkron, ErgodicTheory.OperatorEntropy.cfc_log_diagonal (fun q : nA × nB => eA q.1 * eB q.2),

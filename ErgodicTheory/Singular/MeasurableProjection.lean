...
polarisation converter of `ErgodicTheory.Singular.StarProjectionPolar`
(`ErgodicTheory.starProjection_apply_coord`), now in its a.e. form.
...
This fact is now **proved** in `ErgodicTheory.MeasureTheory.AnalyticUniversallyMeasurable`
...
(`ErgodicTheory.MeasureTheory.AnalyticUniversallyMeasurable`, via Choquet capacitability) for any
s-finite `μ`. -/

...
(`ErgodicTheory.orthProjMatrix_apply`). The polarisation identity
`ErgodicTheory.starProjection_apply_coord` expands that coordinate as a fixed real-arithmetic
...
* `ErgodicTheory.aemeasurable_orthProjMatrix_of_measurableGraph`: **the general a.e. converter** — a
...
Each entry is a projected-basis coordinate (`ErgodicTheory.orthProjMatrix_apply`), which
...
theorem aemeasurable_orthProjMatrix_of_measurableGraph (μ : Measure X) [SFinite μ]
    ...
    AEMeasurable (fun x => orthProjMatrix (V x)) μ := by
...
`ErgodicTheory.MeasureTheory.AnalyticUniversallyMeasurable` and threaded through the converter. -/
theorem aemeasurable_orthProjMatrix_lambdaSublevel
    ...
    AEMeasurable (fun x => orthProjMatrix (lambdaSublevel A T x c)) μ :=

...
* `ErgodicTheory.cocycle_expanding_bound` — the compounded bound `Kⁿ‖v‖ ≤ ‖D(T^[n]) v‖`.
* `ErgodicTheory.expanding_pow_le_singularValues` — `Kⁿ ≤ σᵢ(A⁽ⁿ⁾)` for every `i < d`.
* `ErgodicTheory.log_le_exponents_of_expanding` — every Lyapunov exponent is `≥ log K`.
* `ErgodicTheory.exponents_pos_of_expanding` — every Lyapunov exponent is strictly positive.
* `ErgodicTheory.sumPosExp_eq_sumAllExp_of_expanding` — for an expanding map `∑λ⁺ = ∑λ`.
* `ErgodicTheory.sumPosExp_eq_integral_log_abs_det_of_expanding` — the **Pesin = Rokhlin** identity
  `∑λ⁺ = ∫ log|det Dₓ T| dμ`.

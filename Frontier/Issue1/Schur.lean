...
-- This file is a verbatim graft of `Physlib.Mathematics.SchurTriangulation` (Gordon Hsu, 2025),
-- adapted to plain imports. The original relies on `autoImplicit`; this project disables it
-- globally (lakefile), so we re-enable it for this self-contained grafted file only.

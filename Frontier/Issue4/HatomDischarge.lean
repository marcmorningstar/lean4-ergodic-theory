...
-/
import Frontier.Issue4.RuelleCount
import Frontier.Issue4.SharpCovering
...
of `Frontier.Issue4.margulisRuelle_sharp_of_atomVolProd`.  The capstone there reduces, modulo
`hatom`, the system entropy to the positive-exponent sum `sumPosExp`; the surrounding orbit reduction
...
*same* honest non-compactness regime as the crude bound `Frontier.Issue4.CrudeRuelle`.
...
1. **The one-step sharp local covering count (interface stub).**  The sharp anisotropic Liao–Qiu
   ...
   i.e. `≤ C_d · volProd T n x`, is built in the sibling worktree `Frontier.Issue4.SharpCovering`
   (NOT in HEAD as of the pinned Mathlib `v4.30.0-rc2`: it needs the orthonormal-basis SVD
   factorisation absent there — see `Frontier.Issue4.LocalCovering` for the recorded obstruction).
   It is declared here as the explicit `Prop`-valued interface
   `Frontier.Issue4.SharpLocalCovering` and used as a hypothesis `hcover`, isolating the single
...
2. **The geometric atom-count count (honest non-compactness hypothesis).**  The combinatorial step
   ...
   the noncompact `EuclideanSpace` it requires the honest regime of `Frontier.Issue4.CrudeRuelle`
...
* `Frontier.Issue4.SharpLocalCovering` — the `Prop`-valued interface for the one-step sharp covering
  count `coveringNumber ε (D_x(T^[n]) '' ball) ≤ C · volProd T n x`.
* `Frontier.Issue4.atomCount_le_volProd_of_sharpCovering` — composing the covering stub with the
...
* `Frontier.Issue4.hatom_of_sharpCovering` — discharges the existential `hatom` of the capstone for a
  fixed partition: at the a.e. orbit-rate base point the atom bound holds.
* `Frontier.Issue4.margulisRuelle_sharp` — the **sharp unconditional (modulo the honest distortion
  regime) Margulis–Ruelle inequality** `ksEntropy hT ≤ ∑ λᵢ⁺`, with the one geometric atom isolated
  as `Frontier.Issue4.SharpLocalCovering` (built in the sibling `SharpCovering`).
...
the sibling worktree `Frontier.Issue4.SharpCovering`; as of pinned Mathlib (`v4.30.0-rc2`) only the
isotropic count `≤ (2‖L‖+1)^d` is available in HEAD (`Metric.coveringCount_image_ball_linear_le`,
`Frontier.Issue4.LocalCovering`), so the sharp `∏ᵢ max(1, σᵢ)` count is infrastructure-blocked and
...
covering count `Frontier.Issue4.coveringCount_image_ball_le_volProd`, instantiated at the differential
...
CLM with `toEuclideanLin (cocycle …)`, so `∏ᵢ max(1, σᵢ(L)) = volProd T n x` *definitionally*, giving
...
Mañé/Katok count `hgeoCount` holds at a base point `x`: the non-empty atom count of the `n`-refinement
...
`Frontier.Issue4.ksEntropyPartition_le_sumPosExp_of_atomVolProd`.
...
there is a constant `Ccov ≥ 0` such that, for `μ`-a.e. base point `x`, the geometric Mañé/Katok count
`hgeoCount` holds (`hgeo`), and a sharp covering constant `Ccov' ≥ 0` valid at every such point
(`hcover`).  Then the existential `hatom` of `Frontier.Issue4.margulisRuelle_sharp_of_atomVolProd`
...
log-integrable derivative cocycle, the Kolmogorov–Sinai *system* entropy is bounded by the sum of the
...
* `hgeo` — the **geometric Mañé/Katok atom count** (the honest non-compactness input, same regime as
  `Frontier.Issue4.CrudeRuelle`: a uniform-distortion / compact-carrier hypothesis is unavoidable on
  the noncompact `EuclideanSpace`, Riquelme 2017): for every finite partition `P` there is a constant
...
`sharpLocalCovering_of_coveringCount` (the sibling `Frontier.Issue4.SharpCovering`'s

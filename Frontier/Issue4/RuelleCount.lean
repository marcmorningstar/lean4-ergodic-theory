...
of `Frontier.Issue4.CrudeRuelle` controls the partition entropy by the *dimension times* the uniform
log-derivative bound, `d · B`; this module sharpens the geometric rate from `d · B` to the genuine
positive-exponent sum `sumPosExp = ∑_{λᵢ > 0} λᵢ`, by counting the image of an `ε`-ball under the
`n`-fold iterate `T^[n]` via the **positive-part singular-value product** of the derivative cocycle.
...
2. **The local covering count (interface `coveringCount_image_ball_le_volProd`, stubbed).**  The
   ...
   (`Frontier.Issue4.LocalCovering`).  It is declared here with the agreed signature as the explicit
   hypothesis `hcover`, so the orbit assembly is sorry-free *modulo* that one geometric atom; the
...
We carry the *same* honest non-compactness data as `Frontier.Issue4.CrudeRuelle` (a uniform
...
* `Frontier.Issue4.volProd` — the per-orbit positive-part singular-value product
  `∏_{i<d} max 1 σᵢ(D(T^[n]))`.
* `Frontier.Issue4.one_le_volProd` — the volume factor is `≥ 1`.
* `Frontier.Issue4.log_volProd_eq_sum_posLog` — `log (volProd …) = ∑_{i<d} log⁺ σᵢ`.
* `Frontier.Issue4.tendsto_log_volProd` — the orbit growth rate
  `(1/n) log (volProd …) → sumPosExp`, a.e. (sorry-free).
* `Frontier.Issue4.coveringCount_image_ball_le_volProd` — the **interface stub** for the local
  covering count (built in the sibling `Frontier.Issue4.LocalCovering`).
* `Frontier.Issue4.ksEntropyPartition_le_sumPosExp_of_atomVolProd` — the orbit assembly: a
  base-point `volProd` atom-count bound yields `h(P, T) ≤ sumPosExp`.
* `Frontier.Issue4.margulisRuelle_sharp_of_atomVolProd` — the sharp Margulis–Ruelle inequality

...
import Frontier.Issue4.VolumeDistortion
...
bound of `Frontier.Issue4.LocalCovering` left open (Liao–Qiu,
...
`δ = ε/2`,
`Frontier.Issue4.CoveringFromVolume`'s volume → covering bound divides by `(ε/2)^d · volume(B 0 1)`:
...
* `Frontier.Issue4.svd_exists` — the constructive **singular-value decomposition** `L bᵢ = σᵢ • cᵢ`
  with orthonormal bases `b, c`, identifying `σᵢ` with `LinearMap.singularValues`.
* `Frontier.Issue4.cthickening_image_closedBall_subset_ellipsoid` — the **ellipsoid domination** of
  the thickened image (the geometric heart, the weighted-`L²` triangle inequality).
* `Frontier.Issue4.volume_cthickening_image_closedBall_le_volProd` — the **anisotropic volume**
  `volume(cthickening δ (L '' B(0,ε))) ≤ ∏ᵢ 2(ε σᵢ + δ) · volume(B 0 1)`.
* `Frontier.Issue4.coveringCount_image_ball_le_volProd` — the sharp anisotropic one-step covering

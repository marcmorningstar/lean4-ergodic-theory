...
* `Oseledets.cocycle`: the iterated linear cocycle generated by `A` over `T`.
* `Oseledets.IntegrableLogNorm`: the one-sided integrability hypothesis
  `log⁺ ‖A‖ ∈ L¹(μ)`.
* `Oseledets.instMeasurableSpaceMatrix`: the entrywise (Pi) measurable structure on
  matrices.

## Main results

* `Oseledets.cocycle_add`: the cocycle identity
  `cocycle A T (m + n) x = cocycle A T m (T^[n] x) * cocycle A T n x`.
* `Oseledets.measurable_cocycle`: each iterate of the cocycle is measurable when the

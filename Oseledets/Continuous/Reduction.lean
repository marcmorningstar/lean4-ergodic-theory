...
This file performs the **reduction of the continuous-flow Oseledets multiplicative ergodic
theorem at integer times** to the already-proved discrete-time theorem
...
The only genuinely new ingredient here is the **integrability of the time-`1` log-norms**. The
discrete theorem requires `log⁺ ‖A 1‖ ∈ L¹(μ)` and `log⁺ ‖(A 1)⁻¹‖ ∈ L¹(μ)`; we deduce these
from a *uniform-on-`[0,1]`* integrable dominating function for the continuous family, which is
the natural continuous-flow hypothesis (a continuous-time analogue of the discrete one-sided

...
* `Oseledets.tendsto_div_of_tendsto_div`: a pure real-analysis ratio lemma — if
  `n⁻¹ · a n → L` and `n⁻¹ · r n → R ≠ 0` then `a n / r n → L / R`.
* `Oseledets.roofSum_natCast_eq_birkhoffSum`: the reconciliation of the integer-indexed
...
* `Oseledets.tendsto_roofAverage_ae`: under ergodicity and integrability of `τ`, the roof
  average `n⁻¹ · roofSum n x` converges `μ`-a.e. to `∫ τ` (the ergodic Birkhoff theorem
  applied to the roof).
* `Oseledets.integral_roof_pos`: positivity of `∫ τ` from a uniform lower bound on `τ` and a
  probability measure.
* `Oseledets.returnTime_tendsto_exponent`: the **return-time exponent**. Given any base

...
* `Oseledets.lapCount_unique`: the lap count is the *unique* index sandwiched by the strictly
  increasing return times, `returnTime m x ≤ t < returnTime (m + 1) x → lapCount t x = m`.
* `Oseledets.lapCount_mono`: `lapCount s x ≤ lapCount t x` whenever `0 ≤ s ≤ t` — more elapsed
  flow time completes at least as many laps.
* `Oseledets.lapCount_returnTime_add`: the lap count at a return boundary splits additively,

...
* `Oseledets.norm_pos_of_isUnit_det`: an invertible matrix (`IsUnit M.det`) over a nonempty index
...
* `Oseledets.coverCocycle_suspensionAct_log_discrepancy`: the **`t`-independent additive
...
* `Oseledets.coverCocycle_suspensionAct_tendsto_exponent`: **the limit transfer** (headline). If the

...
`Oseledets.Continuous.SuspensionCoverFlow` does **not** descend to a single matrix on the orbit
...
`Oseledets.Continuous.SuspensionDescent` already handles the **base-section, single-lap** case
...
* `Oseledets.coverCocycle_suspensionAct_rebasing`: the **orbit re-basing identity** (the genuine
...
* `Oseledets.coverCocycle_suspensionAct_opNorm_le`: the forward operator-norm shadow,
  `‖coverCocycle (x, s) t‖ ≤ ‖coverCocycle (suspensionAct (n : ℤ) (x, s)) t‖ * ‖cocycle A T n x‖`.
* `Oseledets.coverCocycle_suspensionAct_rebasing_inv`: under base-cocycle invertibility
...
* `Oseledets.coverCocycle_suspensionAct_opNorm_ge`: the reverse operator-norm shadow under
  invertibility,

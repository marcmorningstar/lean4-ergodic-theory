...
* `Oseledets.measurePreserving_shear`: the shear `(x, s) ↦ (x, s − τ x)` preserves `μ × volume`.
* `Oseledets.measurePreserving_suspensionGen`: the generator `suspensionGen T hτ` preserves
  `μ × volume`, given `MeasurePreserving T μ μ`.
* `Oseledets.suspensionDomain_fiber`: the `x`-fibre of the box is the half-open interval
  `Ico 0 (τ x)`.
* `Oseledets.measure_suspensionDomain`: the box mass equals `ENNReal.ofReal (∫ x, τ x ∂μ)`

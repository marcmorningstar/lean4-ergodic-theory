...
* `Oseledets.SuspensionSpace`: the suspension quotient `Quotient (AddAction.orbitRel ℤ (X × ℝ))`.
* `Oseledets.suspensionMeasure₀`: the raw pushforward measure `(restrict box).map π`.
* `Oseledets.suspensionMeasure`: the normalised invariant probability measure
  `(∫ τ)⁻¹ • suspensionMeasure₀`.

## Main results

* `Oseledets.suspensionMeasure₀_univ`: `suspensionMeasure₀ univ = ENNReal.ofReal (∫ x, τ x ∂μ)`.
* `Oseledets.isProbabilityMeasure_suspensionMeasure`: with `0 < ∫ τ` and `Integrable τ`, the

...
* the **bounded-roof cross-section exponent** `coverCocycle_tendsto_exponent_of_bddRoof`
  (`Oseledets.Continuous.SuspensionBddRoofExponent`), giving for base-`μ`-a.e. `x` the section
...
* the **disintegration / fundamental-domain transfer**
  `ae_suspensionMeasure_section_exponent_set` (`Oseledets.Continuous.SuspensionDisintegration`),
...
* the **`HasFlowExponent` predicate** (`Oseledets.Continuous.SuspensionSpaceExponent`), the
...
* `Oseledets.tendsto_coverCocycle_exponent_of_section`: the height-shift. If
  `Real.log ‖coverCocycle (x, 0) t‖ / t → L`, then `Real.log ‖coverCocycle (x, s) t‖ / t → L` for
  every height `s`.
* `Oseledets.ae_suspensionMeasure_hasFlowExponent`: the **space-level exponent**. Under a bounded

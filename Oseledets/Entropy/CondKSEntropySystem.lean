...
Building on the partition-relative entropy `condKsEntropyPartition hm hT hTA hpull P =
...
The two-sided invariance hypotheses on `T` (`𝒜/𝒜`-measurability and the `T`-preimage pull-back
property) are fixed parameters of the definition; the supremum ranges over `Fin n`-indexed
partitions for every `n : ℕ`. As in the absolute case, the supremum may be infinite, so it is
valued in `EReal`.
...
condKsEntropyPartition hm hT hTA hpull P` over all finite measurable partitions `α`. The two-sided
invariance hypotheses on `T` are fixed parameters; the supremum ranges over `Fin n`-indexed
partitions for every `n : ℕ`. Valuing it in the complete lattice `EReal` makes it total even when
the entropy is unbounded (`+∞`). -/
noncomputable def condKsEntropy {μ : Measure α} [IsProbabilityMeasure μ] {T : α → α}
    (hm : 𝒜 ≤ mα) (hT : @MeasurePreserving α α mα mα T μ μ) (hTA : @Measurable α α 𝒜 𝒜 T)
    (hpull : ∀ A : Set α, MeasurableSet[𝒜] A →
      ∃ A' : Set α, MeasurableSet[𝒜] A' ∧ A =ᵐ[μ] T ⁻¹' A') : EReal :=
  ⨆ n : ℕ, ⨆ P : MeasurePartition μ (Fin n),
    ((condKsEntropyPartition hm hT hTA hpull P : ℝ) : EReal)
...
lemma le_condKsEntropy {μ : Measure α} [IsProbabilityMeasure μ] {T : α → α}
    ...
    {n : ℕ} (P : MeasurePartition μ (Fin n)) :
    ((condKsEntropyPartition hm hT hTA hpull P : ℝ) : EReal) ≤ condKsEntropy hm hT hTA hpull :=
  le_trans (le_iSup (fun P : MeasurePartition μ (Fin n) =>
      ((condKsEntropyPartition hm hT hTA hpull P : ℝ) : EReal)) P)
    (le_iSup (fun n : ℕ =>
      ⨆ P : MeasurePartition μ (Fin n),
        ((condKsEntropyPartition hm hT hTA hpull P : ℝ) : EReal)) n)
...
lemma condKsEntropy_nonneg {μ : Measure α} [IsProbabilityMeasure μ] {T : α → α}
    ...
    (hpull : ∀ A : Set α, MeasurableSet[𝒜] A →
      ∃ A' : Set α, MeasurableSet[𝒜] A' ∧ A =ᵐ[μ] T ⁻¹' A') :
    0 ≤ condKsEntropy hm hT hTA hpull := by
  have h0 : (0 : EReal) ≤
      ((condKsEntropyPartition hm hT hTA hpull (trivialPartition μ) : ℝ) : EReal) :=
    EReal.coe_nonneg.mpr (condKsEntropyPartition_nonneg hm hT hTA hpull (trivialPartition μ))
  exact h0.trans (le_condKsEntropy hm hT hTA hpull (trivialPartition μ))
...
`EReal`-valued suprema therefore coincide. -/
lemma condKsEntropy_bot {μ : Measure α} [IsProbabilityMeasure μ] {T : α → α}
    ...
    condKsEntropy (𝒜 := ⊥) bot_le hT hTA hpull = ksEntropy hT := by
  rw [condKsEntropy, ksEntropy]
  refine iSup_congr fun n => iSup_congr fun P => ?_
  rw [condKsEntropyPartition_bot hT hTA hpull P]

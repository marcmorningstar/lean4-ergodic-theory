...
-/
import Oseledets.Entropy.CondKSEntropyCondBound
...
This module is the **conditional analog** of `Oseledets.Entropy.ProductIdEntropy`
...
`h(T × id | comap fst mX) = 0`  (`Oseledets.Entropy.condKsEntropy_prod_id_eq_zero`).
...
* `Oseledets.Entropy.condKsEntropyPartition_le_ksEntropyPartition`: conditioning does not increase
  the relative Kolmogorov–Sinai entropy.
* `Oseledets.Entropy.condKsEntropyPartition_prodBPart_eq_zero`: the conditional rectangle entropy
  vanishes (B).
* `Oseledets.Entropy.tendsto_condEntropy_prodBPart_sup_zero`: the conditional defect tends to `0`
  (C).
* `Oseledets.Entropy.condKsEntropy_prod_id_eq_zero`: `h(T × id | comap fst mX) = 0` (D).

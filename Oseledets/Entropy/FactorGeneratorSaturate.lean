...
* `Oseledets.Entropy.comap_generatedSigmaAlgebra_pulledBack`: `σ(π⁻¹R) = comap π σ(R)` — the clean
  base lemma identifying the pulled-back generated σ-algebra with the comap of the base one.
* `Oseledets.Entropy.factor_iSup_comap_eq`: `⨆ n, comap (T^[n]) σ(π⁻¹R) = comap π mβ` — the forward

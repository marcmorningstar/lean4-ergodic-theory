...
* `Oseledets.Entropy.generatedSigmaAlgebra_ksJoin_mono`: `σ(B_M) ≤ σ(B_n)` for `M ≤ n`.
* `Oseledets.Entropy.comap_iterate_generatedSigmaAlgebra_ksJoin_le`:
  `comap (T^[a]) σ(B_b) ≤ σ(B_{a+b})`.
* `Oseledets.Entropy.iSup_generatedSigmaAlgebra_ksJoin_eq`:

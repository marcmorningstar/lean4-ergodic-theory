...
* `Oseledets.Entropy.ksEntropySeq`: the entropy sequence `n ↦ H(⋁ₖ₌₀ⁿ⁻¹ T⁻ᵏ α)`.
* `Oseledets.Entropy.ksEntropyPartition`: the Kolmogorov–Sinai entropy `h(α, T)`, the Fekete limit.

## Main results

* `Oseledets.Entropy.ksEntropySeq_subadditive`: `ksEntropySeq (n + m) ≤ ksEntropySeq n +
  ksEntropySeq m`.
* `Oseledets.Entropy.ksSubadditive`: the sequence is a `Subadditive` sequence.
* `Oseledets.Entropy.tendsto_ksEntropySeq`: `n ↦ ksEntropySeq n / n` tends to `ksEntropyPartition`.

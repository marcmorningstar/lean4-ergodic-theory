...
* `Oseledets.Entropy.negMulLog_sum_le_sum_negMulLog`: `negMulLog (∑ⱼ x j) ≤ ∑ⱼ negMulLog (x j)`
  for a nonnegative family.
* `Oseledets.Entropy.entropy_le_entropy_join`: the static refinement bound `H(α) ≤ H(α ∨ β)`.
* `Oseledets.Entropy.ksEntropySeq_le_join`: the per-`n` bound `ksEntropySeq α n ≤
  ksEntropySeq (α ∨ β) n`.
* `Oseledets.Entropy.ksEntropyPartition_le_join`: `h(α, T) ≤ h(α ∨ β, T)`, monotonicity of the

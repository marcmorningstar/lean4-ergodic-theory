...
factor-relative entropy invariance (`Oseledets.Entropy.factor_relative_eq`) the partition-relative
entropy `h(R, T)` equals `h(fst⁻¹ R, T × id)`, and the latter is bounded above by the entropy of the
product system (`Oseledets.Entropy.le_ksEntropy`). Taking the supremum over all base partitions `R`
gives `h(T) ≤ h(T × id)`.

## Main results

* `Oseledets.Entropy.ksEntropy_le_prod`: `h(T) ≤ h(T × id)`.

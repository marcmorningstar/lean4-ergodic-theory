...
* `Oseledets.Entropy.ksEntropySeq_id_of_pos`: for `n ≥ 1`, the `n`-fold iterated-join entropy under
  the identity map equals the static entropy `H(η)`.
* `Oseledets.Entropy.ksEntropyPartition_id_eq_zero`: the identity map has zero partition entropy.
* `Oseledets.Entropy.ksEntropyPartition_rectangle_eq`: the frozen-factor rectangle entropy equals
  the base entropy.

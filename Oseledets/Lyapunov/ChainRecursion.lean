...
lemma slowProj_mem_span (t : ℕ) (hi : Finset (Fin D)) (u : E) :
    S.slowProj t hi u
      ∈ Submodule.span ℝ (Set.range (fun j : (hiᶜ : Finset (Fin D)) => S.e t (j : Fin D))) := by
  classical
  rw [slowProj]
...
lemma norm_fastProj_le (t : ℕ) (hi : Finset (Fin D)) (u : E) :
  ...
  classical
  have h1 : ‖S.fastProj t hi u‖ ^ 2 = ∑ j ∈ hi, ⟪S.e t j, u⟫ ^ 2 := S.normSq_fastProj t hi u
...
/-- **One-step recursion (deterministic).**  Fix consecutive times `t, t+1` with fast bands `hi t`,
`hi (t+1)`.  Assume the slow cap `s` at time `t` (every `σ t j ≤ s` for `j ∈ (hi t)ᶜ`), the fast
floor `tt > 0` at time `t+1` (every `σ (t+1) j ≥ tt` for `j ∈ hi (t+1)`), and the step bound
`‖apply (t+1) w‖ ≤ b·‖apply t w‖` for the slow part `w = slowProj t (hi t) u`.  Then
...
theorem oneStep_recursion (t : ℕ) (hiT hiT1 : Finset (Fin D)) (s tt b : ℝ)
    ...
    (u : E)
    (hstep : ‖S.apply (t + 1) (S.slowProj t hiT u)‖ ≤ b * ‖S.apply t (S.slowProj t hiT u)‖) :
...
/-- **Contraction-free chain solution.**  If `a 0 = 0` and `a (i+1) ≤ a i + R·ρ^i` for all `i` with
`0 ≤ R`, `0 ≤ ρ < 1`, then `a k ≤ R/(1−ρ)` for every `k` — uniformly in `k`.  (Geometric series; no
per-step contraction needed because the source already decays.) -/
theorem chain_geometric_sum (a : ℕ → ℝ) (R ρ : ℝ) (hR : 0 ≤ R) (hρ0 : 0 ≤ ρ) (hρ1 : ρ < 1)
...
`bandProjector A T χ m x = cfc χ (qpow A T m x)`, and `qpow = cfc (·^{1/2m}) (gram)`, so by CFC
composition `bandProjector A T χ m x = cfc (χ ∘ (·^{1/2m})) (gram A T m x)`.  Hence it acts on the
...
@[simp] lemma chainSVD_e [NeZero d] (A : X → Matrix (Fin d) (Fin d) ℝ) (T : X → X) (x : X) (t : ℕ) :
    (chainSVD A T x).e t = sortedGramEigenbasis A T t x := rfl

@[simp] lemma chainSVD_σ [NeZero d] (A : X → Matrix (Fin d) (Fin d) ℝ) (T : X → X) (x : X) (t : ℕ)
    (j : Fin (Fintype.card (Fin d))) :
...
def hiBand [NeZero d] (A : X → Matrix (Fin d) (Fin d) ℝ) (T : X → X) (m : ℕ) (x : X) (c₀ : ℝ) :
...
theorem toEuclideanLin_bandProjector_eq_fastProj [NeZero d]
    (A : X → Matrix (Fin d) (Fin d) ℝ) (m : ℕ) (x : X) (c₀ : ℝ) (u : EuclideanSpace ℝ (Fin d)) :
  ...
  intro j hj
  simp only [Finset.mem_filter] at hj
  have hχ1 : χ (ev j) = 1 := by rw [hχ, Set.indicator_of_mem (show ev j ∈ Set.Ioi c₀ from hj.2) 1,
    Pi.one_apply]
  rw [hχ1, one_smul, hb]

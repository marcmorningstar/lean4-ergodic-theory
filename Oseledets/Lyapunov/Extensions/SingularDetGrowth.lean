...
* `Oseledets.ae_forwardSingularExponent_full_eq_coe` — `γ_d = (Γ_d⁺ : EReal)` `μ`-a.e. (the
  top-index value, forward-only hypotheses).
* `Oseledets.ae_forwardSingularExponent_full_eq_det_growth` — the headline: a single forward

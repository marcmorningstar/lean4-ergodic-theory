...
* `Oseledets.forwardSingularExponent` — the cumulative forward singular exponent `γ_k`, an
  `EReal`-valued `limsup`, defined for every `x` with no invertibility hypothesis.

## Main results

* `Oseledets.measurable_forwardSingularExponent` — `γ_k` is measurable (from `measurable_sprod`
  through the `ℝ → EReal` coercion and `Measurable.limsup`).
* `Oseledets.forwardSingularExponent_nonneg` — `0 ≤ γ_k(x)` for **every** `x` (deterministic:
  each `log⁺`-term is `≥ 0`).
* `Oseledets.forwardSingularExponent_zero` — `γ_0 = 0` everywhere (the empty product is `1`).
* `Oseledets.ae_forwardSingularExponent_eq_coe` — under ergodicity and forward integrability,
  `γ_k = (Γ_k⁺ : EReal)` `μ`-a.e. for a real constant `Γ_k⁺`.
* `Oseledets.ae_forwardSingularExponent_lt_top`, `Oseledets.ae_forwardSingularExponent_ne_bot` —
...
* Everything here rests **only** on the `sprod` forward results of
  `Oseledets/Lyapunov/Extensions/Singular.lean`, which need only forward integrability and
...
* The genuine-`log` `limsup` is identified with `γ_k` only when `Γ_k⁺ > 0`
  (`Oseledets.limsup_logSprod_eq_top_of_pos`); the contracting case `Γ_k⁺ = 0` breaks that

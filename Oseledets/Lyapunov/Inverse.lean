...
* `Oseledets.singularValues_inv` — the **SVD reciprocal-reversed identity**: for an invertible
  matrix `M`, `σᵢ(M⁻¹) = (σ_{rev i}(M))⁻¹`, where `rev` is the order reversal on `Fin d`.
* `Oseledets.singularValues_inv_mul` — the product form `σᵢ(M⁻¹) · σ_{rev i}(M) = 1`.
* `Oseledets.tendsto_log_singularValue_inv_cocycle` — for each sorted index `i` and `μ`-a.e.

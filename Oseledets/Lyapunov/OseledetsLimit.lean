...
/-- **DELIVERABLE 1 — the band projector is the SORTED top-`k` Gram eigenframe projector.** For a
cut
...
/-- **L7c.3c (scalar simplification).** The off-diagonal numerator over the gap denominator
collapses
...
band-projector frame ↔ Plücker eigenvector bridge and the rank-1 lower bound `μ̃₀ ≥ cM²/cBi²` are
not
...
/-- **DELIVERABLE 3 — the per-step cut/gap/regime conditions at a single `n`.** The conjunction of
all
...
`e^{lamSing A T x i}`. This is the eigenvalue half of L9 at the level of the *approximants*; the
full
...
set_option linter.style.longFile 3900

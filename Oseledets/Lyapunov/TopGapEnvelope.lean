...
/-!
# PROVED assembly for `topGapMassEnvelope_ae`

QA-gatekeeper output reconciling Designer 1 (faithful Ruelle) and Designer 2
(abstract deterministic core).  This file is the FULLY PROVED assembly of the
former frozen skeleton: all 14 sub-lemmas now carry complete proofs (spliced from
the 11 prove-wave workers), with ZERO `sorry` and ZERO `sorryAx`.  Every public
lemma's `#print axioms` (see the audit block at the bottom) shows exactly
`[propext, Classical.choice, Quot.sound]`.  The target theorem
`topGapMassEnvelope_ae` composes from the sub-lemmas.

    theorem topGapMassEnvelope_ae : ∀ᵐ x ∂μ, TopGapMassEnvelope A T lam0 x.

## Reconciliation verdict (which design won, and why)

The skeleton is built on **Designer 2's three-layer factorization**, for two
decisive modularity reasons confirmed by adversarial QA:

* PART A (`ChainCore`) is FULLY PROVED (zero sorry) and gives the strongest abstract
  statement: `multi_source_envelope` converts the per-step multi-source recursion
  directly into the m-uniform per-stratum-rate sum.  Designer 1's
  `chain_partialSum_le`/`geom_source_sum_le` are subsumed by it.
* PART B is **measure-free**: the dynamical facts enter only as the plain predicates
  `Localized` / `Tempered`, so the eight B-lemmas (incl. the two crux nodes) can be
  proved by workers who never touch measure theory.  Designer 1 threaded a single
  `GoodPoint` (measure-flavoured) hypothesis through the whole stack, coupling the
  analytic induction to the a.e. layer.

**Repair adopted from Designer 1 / D2's adversarial re-derivation (load-bearing):**
the single localization slack must be `η ≤ min(δ, G)/16`, STRICTER than Designer 1's
`min(δ/8, G/8)`.  Designer 1's η is unsound for the intermediate sub-band inclusion
`stratumBand w ⊆ hiBand(m, canonCut_w)` (it needs `η < δ*_w`, and `δ*_w` can be as
small as `δ/16`).  See the η-budget block in `perStratum_step` (B6) below.

## The SINGLE η budget (re-verified line by line in QA; the heart of the proof)

Fix the pair `(λ_a, v)`, `δ* = canonSlack = min(δ/4, G/2, (v−λ_a)/4)`, canonical cut
`log c_v = v − δ*`, and δ' = δ/4 for the IH.  Choose ONE slack `η ≤ min(δ, G)/16`.
This single η simultaneously discharges all SIX constraints (worst cases shown; uses
`w ≤ pred(v) ≤ v − G` for slow strata and `v − λ_a ≥ G`):
...
4. **canonical-cut gapsep** (`mem_hiBand_iff_of_localized` at γ = v − δ*): slow side
   `η < (v−δ*) − λ_j ≥ G/2`; fast side (incl. `λ_j = v`) `η < δ* + (λ_j − v)`, worst
   `λ_j = v` gives `η < δ*`; both ⟸ `η ≤ min(δ,G)/16 < δ*`.  ✓
5. **intermediate sub-band inclusion** `stratumBand w ⊆ hiBand(m, canonCut_w)`: need
   `η < δ*_w = min(δ'/4, G/2, (w−λ_a)/4) ≥ min(δ/16, G/4)`; `η ≤ min(δ,G)/16`.  ✓
   THIS is the binding constraint (Designer 1's η fails it).
6. **init `a_0 = 0`** (`canonCut_init_zero`): need `λ_a + η < v − δ*`, i.e.
   `η < (v−λ_a) − δ* ≥ G/2`.  ✓

Note `η < δ*` always holds: `δ* = min(δ/4, G/2, (v−λ_a)/4) ≥ min(δ/4, G/4)` (using
`v−λ_a ≥ G`), and `η ≤ min(δ,G)/16 < min(δ/4, G/4) ≤ δ*`.

Conventions mirror the committed code: `Fintype.card (Fin d) = d` casts via
`⟨(a : ℕ), lt_of_lt_of_eq a.isLt (Fintype.card_fin d).symm⟩`; `hiBand` membership is
`σ_j(t)^{1/t} > c`; `bandProjector` cuts the EXP scale.

KNOWN TRAPS baked in (see the briefing):
1. δ*-quantization is against the minimal DISTINCT-stratum gap `G` (`exists_distinctGap`),
   never against raw index-adjacent gaps.
2. qpow scale: never mix `σ^{1/t}` and `σ^{1/(t+1)}`; convert at each fixed time.
3. cut stays STRICTLY gap-interior.
4. all `Eventually` thresholds collected once and threaded as a single `n₀`.
5. the envelope is TWO-TIME: `n` and ALL `m ≥ n` with the SAME `n`.
6. the single η is `≤ min(δ,G)/16` (D1's `min(δ/8,G/8)` is UNSOUND — trap 5 above).
-/
...
`k`, then `a k ≤ ∑_{i<k} src i` for every `k`.  (Elementary induction; this replaces the
heavier `chain_geometric_sum` — we want the raw partial-sum form so the geometric bound is
applied separately, exactly as KEY INSIGHT 2 prescribes.) -/
...
/-- The time-`m` sub-band of stratum value `v`: indices `j` whose Lyapunov value `lam0 j` equals `v`.
This is a TIME-INDEPENDENT finset (it depends only on `lam0`), but we keep `m` for symmetry with
`hiBand`; the localization hypothesis is what ties it to the actual time-`m` band. -/
...
theorem subband_mass_le_bandMass [NeZero d] (A : X → Matrix (Fin d) (Fin d) ℝ) (m : ℕ) (x : X)
  ...
  have h0 : ‖S.fastProj m B u‖ ^ 2 = ∑ j ∈ B, ⟪S.e m j, u⟫ ^ 2 := S.normSq_fastProj m _ u
...
avoids every stratum and no stratum lies in `(?, log c]∖{strata}`… more precisely: the complement of
`hiBand A T t x c` is `{j : lam0 j < log c}` (membership characterization).  Stated as the membership
equivalence at a single time `t` past the localization threshold, with `c = exp γ`, `γ` strictly
gap-separated from every stratum by more than `η`. -/
theorem mem_hiBand_iff_of_localized [NeZero d] {A : X → Matrix (Fin d) (Fin d) ℝ}
    ...
    {t : ℕ} (ht : N ≤ t) (ht1 : 1 ≤ t) (j : Fin (Fintype.card (Fin d))) :
...
theorem stratum_sigma_cap [NeZero d] {A : X → Matrix (Fin d) (Fin d) ℝ} (hA : ∀ x, (A x).det ≠ 0)
    (x : X) (lam0 : ℕ → ℝ) {η : ℝ} (hη : 0 < η) {N : ℕ} (hloc : Localized A T x lam0 η N)
    {w : ℝ} {t : ℕ} (ht : N ≤ t) (ht1 : 1 ≤ t)
    (j : Fin (Fintype.card (Fin d))) (hj : j ∈ stratumBand lam0 w) :
    (Matrix.toEuclideanLin (cocycle A T t x)).singularValues j ≤ Real.exp ((t : ℝ) * (w + η)) := by
  ...
  have hpow : (Matrix.toEuclideanLin (cocycle A T t x)).singularValues j ≤ Real.exp (w + η) ^ t :=
...
This is the refinement of the committed single-cap `bandMass_oneStep_recursion`.  Instead of
bounding the slow-projection norm `‖slowProj_t u‖` by a single cap, we split the slow band into
per-stratum sub-bands and bound the leakage source as a SUM over strata, each with its own sub-band
mass and its own σ-cap.  Two routes (we pick the triangle-inequality route): write
`slowProj_t u = ∑_w fastProj_t (B_w) u` over the partition `{B_w}` of `(hiBand)ᶜ`, then the operator
step bound distributes; each sub-band term is bounded via `oneStep_sandwich` applied to that
sub-band's span (each `fastProj_t (B_w) u` lies in the span of `e_t` over `B_w ⊆ (hiBand)ᶜ`). -/
...
open Ruelle13 in
/-- **STEP 1 — the SVDData-level multi-source one-step recursion.**  Generalizes
...
theorem bandMass_oneStep_multisource [NeZero d] {W : Type*} (A : X → Matrix (Fin d) (Fin d) ℝ)
  ...
  classical
  -- STEP 2: wire the SVDData-level multi-source recursion to the cocycle band level.
  set S := chainSVD A T x with hS
...
CRITICAL design choice: the cut is NOT a free parameter inside the predicate; it is the canonical
cut `canonCut a v δ = exp(v − δ*)` with `δ* = min(δ/4, G/2, (v − lam0 a)/4)`.  This is KEY INSIGHT 1
— running the recursion at this δ-dependent cut is what makes the induction close.  We carry the
distinct-stratum gap `G = distinctGap lam0` (TRAP 1) as a parameter. -/

/-- **B-gap — existence of a positive distinct-stratum gap** (TRAP 1: slacks are quantized against
THIS, never raw index-adjacent gaps).  There is `G > 0` below every gap between DISTINCT stratum
values: `lam0 i < lam0 j ⟹ lam0 i + G ≤ lam0 j`.  (The min over the finitely many positive
...
theorem exists_distinctGap [NeZero d] (lam0 : ℕ → ℝ) :
    ∃ G : ℝ, 0 < G ∧ ∀ i j : Fin d, lam0 (i : ℕ) < lam0 (j : ℕ) → lam0 (i : ℕ) + G ≤ lam0 (j : ℕ) := by
  classical
...
/-- The canonical δ-dependent slack `δ*` for the pair `(lam0 a, v)` (KEY INSIGHT 1): strictly
positive, ≤ δ/4, ≤ G/2, ≤ (v − lam0 a)/4. -/
def canonSlack (lam0 : ℕ → ℝ) (a : ℕ) (v G δ : ℝ) : ℝ :=
  min (min (δ / 4) (G / 2)) ((v - lam0 a) / 4)

/-- The canonical cut for the pair `(lam0 a, v)` at slack `δ`: `exp(v − δ*)`, strictly interior to the
gap below `v` (since `δ* ≤ G/2 < G` and the predecessor stratum is `≤ v − G`). -/
...
theorem canonCut_init_zero [NeZero d] {A : X → Matrix (Fin d) (Fin d) ℝ}
    (hA : ∀ x, (A x).det ≠ 0) (x : X) (lam0 : ℕ → ℝ) (a : Fin d) {v G δ η : ℝ} {N n : ℕ}
    (hloc : Localized A T x lam0 η N) (hn : N ≤ n) (hn1 : 1 ≤ n)
    (hav : lam0 (a : ℕ) < v) (hη : 0 < η)
...
Mechanism (KEY INSIGHT 1+2, re-derived):
* Run the recursion at `c = canonCut a v G δ = exp(v − δ*)`, `δ* = canonSlack`.
* The slow band of `c` at time `m` is (by `mem_hiBand_iff_of_localized`) exactly the disjoint union of
  `stratumBand lam0 w` over distinct strata `w < v − δ*`, i.e. all distinct `w ≤ pred(v) < v`.
* Per-step source for stratum `w` (B4 term):
  ...
  `δ* ≤ G/2`, so `(v−δ*)−w ≥ G/2 > 0`; satisfied by the SINGLE `η ≤ min(δ,G)/16` below — do NOT use
  the looser `G/8`, see η-budget point 5 in the file header).
* mass_w(m) bound:
  ...
  - intermediate strata `lam0 a < w < v`: by the IH `PerStratumEnvelope … a w` at `δ' = δ/4`,
    `mass at ITS canonical cut canonCut_w = exp(w − δ*_w) ≤ C_w·exp(−n(w − lam0 a − δ'))`, UNIFORMLY
    in `m ≥ n`; transfer `mass_w(m) ≤ ‖P^{>canonCut_w}_m u_a(n)‖` via B1, which needs
    ...
    take `η ≤ min(G, δ)/16` (ADVERSARIAL CHECK: this is STRICTER than the `G/8` above; the binding
    constraint is the intermediate sub-band inclusion, not the `ρ_w < 1` one).  Geometric source then
    has `Mw = C_w·exp(−n(w − lam0 a − δ'))·exp(−(v−δ*))` and the m-sum gives
    `≈ K_w·exp(−n((v−δ*) − w − 2η))·exp(−n(w − lam0 a − δ'))`
    `= K_w·exp(−n(v − lam0 a − δ* − δ' − 2η)) ≤ K_w·exp(−n(v − lam0 a − δ))`
    (δ* + δ' + 2η ≤ δ/4 + δ/4 + δ/8 ≤ δ). ✓
* Assemble via A4 (multi-source envelope): `a_k ≤ ∑_w (Mw/(1−ρw))·ρw^n`, each term `≤ C_w'·exp(−n·rate_v)`
...
theorem perStratum_step [NeZero d] {A : X → Matrix (Fin d) (Fin d) ℝ} (hA : ∀ x, (A x).det ≠ 0)
  ...
  have hrec : ∀ k : ℕ, aseq (k + 1) ≤ aseq k + ∑ w ∈ Wf, srcw w k := by
    ...
    show aseq (k + 1) ≤ aseq k + ∑ w ∈ Wf, srcw w k
  ...
  have hsrc : ∀ w ∈ Wf, ∀ k : ℕ, srcw w k ≤ Mw w * ρw w ^ (n + k) := by
    ...
    have hcoeff_nonneg : 0 ≤ ‖A (T^[n + k] x)‖ * Real.exp ((↑(n + k) : ℝ) * (w + η)) / c ^ (n + k + 1) := by
  ...
  have hterm : ∀ w ∈ Wf,
    ...
    have hnum : Mw w * ρw w ^ n
      ...
      · rw [if_pos hlt]
        ...
        have hcombine : Real.exp (-(v - δstar)) * (Cfun w * Real.exp (-(n : ℝ) * (w - lam0 (a : ℕ) - δ')))
...
private lemma htgind_aux [NeZero d] {A : X → Matrix (Fin d) (Fin d) ℝ}
  ...
  | zero =>
    ...
    -- The set of intermediate strata is empty (card ≤ 0), so the IH for `perStratum_step` is vacuous.
  ...
  | succ M ih =>
    ...
    refine perStratum_step hA x lam0 hG hGgap hconv htemp a v hav hvstratum ?_
    intro w hlt hwv hex
    -- `htgind_S lam0 a w ⊂ htgind_S lam0 a v`: subset by transitivity, strict because `w ∈ Sᵥ \ S_w`.
...
/-! ### Band stabilization (KEY INSIGHT 3)
...
threshold.  Hence two gap-interior cuts to the SAME top gap below `lam0 e` give EQUAL bands, so equal
band-projector masses. -/
...
theorem bandMass_eq_of_gap_interior [NeZero d] {A : X → Matrix (Fin d) (Fin d) ℝ}
    ...
    (hsame : ∀ j : Fin (Fintype.card (Fin d)), Real.log c₀ < lam0 (j : ℕ) ↔ Real.log c₁ < lam0 (j : ℕ))
...
theorem topGap_bound_at_cut_of_perStratum [NeZero d] {A : X → Matrix (Fin d) (Fin d) ℝ}
    ...
    (henv : PerStratumEnvelope A T lam0 x G a (lam0 (e : ℕ)))
    {δ : ℝ} (hδ : 0 < δ) {C : ℝ} (hC1 : 1 ≤ C)
    (hCenv : ∀ᶠ n : ℕ in Filter.atTop, ∀ m : ℕ, n ≤ m →
      ‖Matrix.toEuclideanLin
          (bandProjector A T (Set.indicator (Set.Ioi (canonCut lam0 (a : ℕ) (lam0 (e : ℕ)) G δ)) 1) m x)
          (sortedGramEigenbasis A T n x
            ⟨(a : ℕ), lt_of_lt_of_eq a.isLt (Fintype.card_fin d).symm⟩)‖
  ...
  -- The decisive same-membership pattern: both cuts agree with the threshold `lam0 e ≤ lam0 j`.
  -- forward at the EXP/real-index level; we also produce per-index facts.
  -- For each i : Fin d, classify by hc₀gap, then deduce both `log c₀ ?? lam0 i` and `log κ ?? lam0 i`.
  have hclass : ∀ i : Fin d,
      (lam0 (e : ℕ) ≤ lam0 (i : ℕ) ∧ Real.log c₀ < lam0 (i : ℕ) ∧ Real.log κ < lam0 (i : ℕ))
        ∨ (lam0 (i : ℕ) < Real.log c₀ ∧ lam0 (i : ℕ) < Real.log κ
            ∧ lam0 (i : ℕ) < lam0 (e : ℕ) ∧ ¬ (lam0 (e : ℕ) ≤ lam0 (i : ℕ))) := by
    intro i
    rcases hc₀gap i with hlo | hhi
    · -- lam0 i < log c₀, so lam0 i < lam0 e ⟹ lam0 i + G ≤ lam0 e ⟹ lam0 i ≤ lam0 e − G ≤ log κ - G/2
      right
...
theorem topGapMassEnvelope_of_perStratum [NeZero d] {A : X → Matrix (Fin d) (Fin d) ℝ}
  ...
  classical
  intro δ hδ
  -- The per-pair canonical-cut constant, built BEFORE refining the ∃C (VACUOUS-PAIR TRAP):
  -- for a gap pair use the constant produced by the per-stratum envelope; otherwise `1`.
...
/-- **TARGET — `topGapMassEnvelope_ae`.**  The last analytic node: a.e. the top-gap fast-band-mass
envelope holds, discharging `htopgap` in `forward_graded_overlap'`. -/
...
/-! ## Axiom / composition audit (all public lemmas must be clean)

Every public lemma below depends on EXACTLY `[propext, Classical.choice, Quot.sound]`.
No `sorryAx` appears anywhere: all 14 nodes are fully proved and spliced in. -/

#print axioms Oseledets.exists_localized
#print axioms Oseledets.exists_tempered
#print axioms Oseledets.subband_mass_le_bandMass
#print axioms Oseledets.mem_hiBand_iff_of_localized
#print axioms Oseledets.stratum_sigma_cap
#print axioms Oseledets.bandMass_oneStep_multisource
#print axioms Oseledets.exists_distinctGap
#print axioms Oseledets.canonCut_init_zero
#print axioms Oseledets.perStratum_step
#print axioms Oseledets.perStratum_strongInduction
#print axioms Oseledets.bandMass_eq_of_gap_interior
#print axioms Oseledets.topGap_bound_at_cut_of_perStratum
#print axioms Oseledets.topGapMassEnvelope_of_perStratum
#print axioms Oseledets.topGapMassEnvelope_ae

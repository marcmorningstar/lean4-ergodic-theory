...
(`Oseledets.Multifractal.condKsEntropy_bernSuspensionFlow_one_baseProj_eq_zero`).

The proof transports the conditional frozen-product vanishing
`Oseledets.Entropy.condKsEntropy_prod_id_eq_zero` (`h(T × id | comap fst) = 0`) across the
fundamental-domain measurable conjugacy `suspensionUnitMeasurableEquiv`, using the conditional
conjugacy-invariance `Oseledets.Entropy.condKsEntropy_congr_of_conjugacy`. The two conditioning
...
* `Oseledets.Multifractal.suspensionBaseProj_eq_fst_comp_unitEquiv`: the base projection is the
  first coordinate of the fundamental-domain equivalence (F-prep).
* `Oseledets.Multifractal.condKsEntropy_bernSuspensionFlow_one_baseProj_eq_zero`: the conditional

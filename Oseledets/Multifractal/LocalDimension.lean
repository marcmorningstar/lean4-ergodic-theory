...
* `Oseledets.Multifractal.localDimension` — the upper local/pointwise dimension `d̄_μ(x)`,
...
* `Oseledets.Multifractal.ae_tendsto_localDimension_of_absolutelyContinuous` — the headline
  a.e.-convergence of the local-dimension quotient to `finrank ℝ E`.
* `Oseledets.Multifractal.ae_localDimension_eq_finrank` — the corollary

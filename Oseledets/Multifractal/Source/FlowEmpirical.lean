...
* `Oseledets.Multifractal.empiricalCellMass` (def): the finite-time empirical cell mass, a
...
* `Oseledets.Multifractal.integral_empiricalCellMass_eq` (**the non-vacuity anchor**): for
...
* `Oseledets.Multifractal.empiricalCellMass_natCast_eq`: at *integer* times the empirical mass
...
* `Oseledets.Multifractal.integral_unitWindow_eq`: the unit-window space average equals the true
  mass, `∫_x unitWindow φ C x ∂μ = (μ C).toReal` (same Fubini computation, `T = 1`).

* `Oseledets.Multifractal.tendsto_empiricalCellMass_ae`: under an ergodicity hypothesis on the

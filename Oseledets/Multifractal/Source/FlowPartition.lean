...
* `Oseledets.Multifractal.dynamicalRefine`: the depth-`n` flow-refined partition, the join over
  `k = 0, …, n-1` of the pullbacks `(φ (k • Δ))⁻¹ P` of a supplied seed partition `P`, realized
  with the flat `Fin n → ι` index discipline of `Oseledets.Entropy.ksJoin`.
* `Oseledets.Multifractal.cellMassFamily`: the cell-mass family `i ↦ (μ (P.cells i)).toReal`, the
  whole #16 ↔ #18 interface.
* `Oseledets.Multifractal.IsHeterogeneous`: the honest non-uniformity hypothesis on `P` — two
  cells of distinct mass. Its failure is *exactly* the equal-measure monofractal degeneracy of #16.
* `Oseledets.Multifractal.finiteTimePartitionEntropy`: the depth-`n` (pre-limit) Shannon entropy
  of `dynamicalRefine`, distinct from its Kolmogorov–Sinai (Fekete) limit.
* `Oseledets.Multifractal.finiteTimeFlowExponent`: the finite-time (pre-limit) Lyapunov estimator
  `log ‖coverCocycle (x, s) t‖ / t`, the function inside the `Tendsto` of `HasFlowExponent`.

## Main results

* `Oseledets.Multifractal.cellMassFamily_sum_eq_one`: the cell masses sum to `1` (genuine
...
noncomputable def finiteTimePartitionEntropy {ι : Type*} [Fintype ι]
  ...
  Oseledets.Entropy.entropy μ (dynamicalRefine φ P Δ n).cells
...
lemma finiteTimePartitionEntropy_def {ι : Type*} [Fintype ι]
    ...
    finiteTimePartitionEntropy φ P Δ n
      = Oseledets.Entropy.entropy μ (dynamicalRefine φ P Δ n).cells := rfl

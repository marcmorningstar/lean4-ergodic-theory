...
* `Oseledets.OperatorEntropy.Lieb.cpow`: the entire complex power `A^{z}` via the spectral theorem.
* `Oseledets.OperatorEntropy.Lieb.upow`: the modular unitary `A^{it} = cpow hA (t·i)`.
* `Oseledets.OperatorEntropy.Lieb.IntertwinesIt`: the imaginary-axis intertwining condition.

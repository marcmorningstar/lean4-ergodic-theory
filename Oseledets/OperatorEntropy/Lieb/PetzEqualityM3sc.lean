...
The linchpin is `equality_imp_intertwinesIt`: if the Umegaki relative entropy is preserved by a
channel `Λ` (`D(ρ‖σ) = D(Λρ‖Λσ)`) then `Λ` **intertwines the modular `it`-flows** of the input and
output pairs (`IntertwinesIt`).  The strict-log keystone (operator strict convexity of `-log`) is
threaded as a hypothesis `hSC : OperatorStrictConvexOn (Set.Ioi 0) (fun x => -Real.log x)` so it can

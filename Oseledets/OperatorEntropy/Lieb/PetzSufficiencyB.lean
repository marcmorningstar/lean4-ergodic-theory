...
* `Oseledets.OperatorEntropy.Lieb.resolvent_linearIndependent`: the resolvent functions
  `x ↦ (t ↦ (x + t)⁻¹)` are `ℂ`-linearly independent over a finite set of distinct positive reals.
* `Oseledets.OperatorEntropy.Lieb.resolvent_span_top`: their evaluation family spans the full
  finite-dimensional function space.
* `Oseledets.OperatorEntropy.Lieb.exists_resolvent_combo`: every target function on the finite set

...
* `Oseledets.OperatorEntropy.Lieb.sum_corner_loewner`: the block corner-pinching inequality.
* `Oseledets.OperatorEntropy.Lieb.rect_isometry_neg_log_loewner`: the rectangular `-log`
...
lemma cfc_fromBlocks_diag {A : Matrix q q ℂ} {B : Matrix r r ℂ}
  ...
  have hdecomp : fromBlocks A 0 0 B = W * fromBlocks (diagonal (fun i => (hAh.eigenvalues i : ℂ)))
    ...
    rw [← hspecA, ← hspecB]
  rw [hdecomp, Oseledets.OperatorEntropy.cfc_conj W _ hW1 hW2 hDsa f, fromBlocks_diagonal]
...
theorem sum_corner_loewner {I : Set ℝ} {f : ℝ → ℝ} (hf : OperatorConvexOn I f)
  ...
  have hcfcVMV : cfc f (diagonal (sgn q r) * M * diagonal (sgn q r))
    ...
    have h := Oseledets.OperatorEntropy.cfc_conj (diagonal (sgn q r)) M

...
* `Oseledets.backwardGen`: the backward generator `x ↦ (A (T.symm x))⁻¹`.
* `Oseledets.BackwardData`: the bundled standing hypotheses for the backward system.

## Main results

* `Oseledets.backwardGen_det_ne_zero`, `Oseledets.measurable_backwardGen`: hypothesis
  transfer (invertibility and measurability) for the backward generator.
* `Oseledets.integrableLogNorm_backwardGen`,

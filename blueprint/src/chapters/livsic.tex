...
\paragraph{The remaining wall.} Only the \emph{trivial} (obstruction) direction of the flow Livšic
theorem is delivered. The converse --- vanishing of all closed-orbit integrals implies \(F\) is a
\emph{Hölder} flow coboundary --- is \textbf{not} proved: its genuine obstacle is the Hölder gluing
of the reconstructed transfer function across the fundamental-domain seam \(s = \tau\), which lies
beyond this regularity-free layer. It is recorded here as a disclosed open front, the flow analogue
of the fully proved discrete equivalence \Cref{thm:livsic-iff}.

...
\begin{theorem}[Deterministic singular-value exponents]\label{exists_lam_tendsto_singularValue}
  \lean{ErgodicTheory.exists_lam_tendsto_singularValue}\leanok
...
\begin{lemma}[Block-value step function reproduces the spectrum]\label{stepVal_exp_lam}
  \lean{ErgodicTheory.stepVal_exp_lam}\leanok
...
\begin{lemma}[Spectral deviation bound]\label{norm_sub_cfc_le_sum_eigenvalue_dev}
  \lean{ErgodicTheory.norm_sub_cfc_le_sum_eigenvalue_dev}\leanok
...
\begin{lemma}[Per-term band-projector convergence]\label{ae_forall_tendsto_block_term}
  \lean{ErgodicTheory.ae_forall_tendsto_block_term}\leanok
...
\begin{theorem}[Existence of the Oseledets limit]\label{tendsto_qpow}
  \lean{ErgodicTheory.tendsto_qpow}\leanok
...
\begin{definition}[The named Oseledets limit]\label{oseledetsLimit}
  \lean{ErgodicTheory.oseledetsLimit}\leanok
...
\begin{theorem}[The limit is the a.e.\ limit of the approximants]\label{tendsto_oseledetsLimit}
  \lean{ErgodicTheory.tendsto_oseledetsLimit}\leanok
...
\begin{proposition}[Structure of the limit]\label{oseledetsLimit_posSemidef}
  \lean{ErgodicTheory.oseledetsLimit_posSemidef}\leanok
...
\begin{lemma}[Gram quadratic-form band bound]\label{inner_cfc_ge_band}
  \lean{ErgodicTheory.inner_cfc_ge_band}\leanok
...
\begin{lemma}[Band lower bound for the cocycle]\label{cocycle_apply_sq_ge_band}
  \lean{ErgodicTheory.cocycle_apply_sq_ge_band}\leanok
  For \(c\ge0\) and \(n\ge1\),
  \[
    c^{2n}\,\norm{P^{c}_n(x)\,v}^2 \;\le\; \norm{A^{(n)}(x)\,v}^2 .
  \]
...
\begin{lemma}[The band correction vanishes]\label{tendsto_inv_mul_log_norm_bandProjector_apply}
  \lean{ErgodicTheory.tendsto_inv_mul_log_norm_bandProjector_apply}\leanok
...
\begin{proposition}[Per-vector liminf lower bound]\label{log_le_liminf_log_cocycle_apply}
  \lean{ErgodicTheory.log_le_liminf_log_cocycle_apply}\leanok
...
\begin{lemma}[Band-projector nesting, kernel propagation]\label{limitBandProjector_apply_eq_zero_of_le}
  \lean{ErgodicTheory.limitBandProjector_apply_eq_zero_of_le}\leanok
...
The upper half is the genuinely non-elementary step. On the ultrametric-growth good set the
per-vector upper growth exponent \(\overline\lambda(x,v)\) of a vector \(v\) in the \(i\)-th
stratum equals its spectral value \(\mathrm{specList}\,i\) exactly, so in particular
\(\limsup_n\tfrac1n\log\norm{A^{(n)}(x)v}\le\mathrm{specList}\,i\). This replaces an earlier
determinant/volume-squeeze argument --- the two \emph{superseded-route} lemmas below, whose
restricted-operator-norm files were removed from the development as dead code; their labels are
retained only so cross-references resolve.

\begin{lemma}[Tempering]\label{tempering_posLog}
  \lean{ErgodicTheory.ae_tendsto_orbit_div_atTop_zero}\leanok
  If \(T\) is measure-preserving and \(g\in L^1(\mu)\), then for \(\mu\)-a.e.\ \(x\),
  \(\tfrac1n\,g(T^n x)\to 0\).
\end{lemma}
\begin{proof}\leanok
...
\begin{lemma}[Slow-volume exponent squeeze (superseded route)]\label{limsup_topSlow_le_of_squeeze}
  \emph{This lemma belonged to a superseded determinant/volume-squeeze route and has no counterpart
  in the current Lean development; the statement is kept for exposition and the label only so that
...
\begin{theorem}[Spectral upper bound on each stratum]\label{spectral_upper_bound_of_squeeze}
  \lean{ErgodicTheory.limsup_log_norm_cocycle_apply_le_specList_of_mem_stratum}\leanok
  For \(\mu\)-a.e.\ \(x\), every index \(i\) and every vector \(v\) in the \(i\)-th stratum
  \(\mathrm{vflag}\,i.\mathrm{castSucc}\setminus\mathrm{vflag}\,i.\mathrm{succ}\) one has
  \[
    \limsup_{n}\ \tfrac1n\log\norm{A^{(n)}(x)\,v} \;\le\; \mathrm{specList}\,i.
  \]
\end{theorem}
\begin{proof}\leanok
  \uses{tempering_posLog}
  On the \(\mathrm{IsUltrametricGrowth}\) good set the per-vector upper growth function
  \(\overline\lambda(x,v)\) equals the exact stratum exponent \(\mathrm{specList}\,i\); since this
  \(\overline\lambda\) is by definition the \(\limsup\) of \(\tfrac1n\log\norm{A^{(n)}v}\), the
  bound follows (indeed with equality). The good set itself is built from the subexponential
  one-step log-norm factor supplied by the tempering estimate (Lemma~\ref{tempering_posLog}), so
  the argument is non-circular.
\end{proof}

\begin{theorem}[Upper bound on the slow space]\label{vslow_subset_lambdaSublevel_of_upper}
  \lean{ErgodicTheory.vslow_subset_lambdaSublevel_of_upper}\leanok
...
\begin{theorem}[Band projectors converge to the CFC indicator]\label{ae_tendsto_bandProjector_cfc_indicator}
  \lean{ErgodicTheory.ae_tendsto_bandProjector_cfc_indicator}\leanok
...
\begin{theorem}[Reverse slow-flag inclusion]\label{ae_lambdaSublevel_le_vslow}
  \lean{ErgodicTheory.ae_lambdaSublevel_le_vslow}\leanok
...
\begin{theorem}[The slow flag equals the limsup sublevel]\label{vslow_eq_lambdaSublevel_of_upper}
  \lean{ErgodicTheory.vslow_eq_lambdaSublevel_of_upper}\leanok
...
\begin{lemma}[Ruelle reverse cofactor bound]\label{entry_reverse_bound_of_orthogonal}
  \lean{ErgodicTheory.RuelleCofactor.entry_reverse_bound_of_orthogonal}\leanok
...
\begin{definition}[Top-gap fast-band-mass envelope]\label{TopGapMassEnvelope}
  \lean{ErgodicTheory.TopGapMassEnvelope}\leanok
...
\begin{lemma}[Multi-source geometric envelope]\label{multi_source_envelope}
  \lean{ErgodicTheory.ChainCore.multi_source_envelope}\leanok
...
\begin{lemma}[Per-stratum envelope step]\label{perStratumEnvelope_step}
  \lean{ErgodicTheory.perStratumEnvelope_step}\leanok
...
\begin{theorem}[The top-gap envelope, a.e.]\label{topGapMassEnvelope_ae}
  \lean{ErgodicTheory.topGapMassEnvelope_ae}\leanok
...
\begin{lemma}[Spectrum identity from two inclusions]\label{lyapunovSpectrum_eq_of_subsets}
  \lean{ErgodicTheory.lyapunovSpectrum_eq_of_subsets}\leanok
...
\begin{theorem}[Ergodic constancy of the spectrum]\label{lyapunovSpectrum_eq_distinctExp_of_lambdaBar}
  \lean{ErgodicTheory.lyapunovSpectrum_eq_distinctExp_of_lambdaBar}\leanok
...
\begin{theorem}[Filtration from the spectral upper bound]\label{oseledets_filtration_of_upper}
  \lean{ErgodicTheory.oseledets_filtration_of_upper'}\leanok
...
\begin{theorem}[Filtration from the top-gap envelope]\label{oseledets_filtration_of_topgap}
  \lean{ErgodicTheory.oseledets_filtration_of_topgap}\leanok
...
\begin{theorem}[One-sided Oseledets multiplicative ergodic theorem]\label{oseledets_filtration}
  \lean{ErgodicTheory.oseledets_filtration}\leanok
